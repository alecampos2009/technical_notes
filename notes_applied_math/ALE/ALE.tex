...
\end{equation}
The Jacobian $\Jvec^{(e)} = \Jvec^{(e)}(t,\zvec)$ is defined as $\Jvec^{(e)} = \partial \xvec^{(e)} / \partial \zvec$, and label its determinant as $J^{(e)} = J^{(e)}(t,\zvec)$. Using \cref{eq:ref_elem_z_map} in the definition of $\Jvec^{(e)}$ we get
\begin{align}
...
A Lagrangian variable $f^+=f^+(t,\yvec)$ has a corresponding reference-element function $f^{(e)} = f^{(e)}(t,\zvec)$, which satisfies
\begin{equation}
    \label{eq:ref_elem_to_lag}
    f^{(e)}(t,\zvec) = f^+(t,\xvec^{(e)}(0,\zvec)).
\end{equation}
Now, $f^+(t,\xvec^{(e)}(0,\zvec)) = f(t,\xvec^+(t,\xvec^{(e)}(0,\zvec)))$. Using \cref{eq:ref_elem_z_map}, we also get
\begin{equation}
    \label{eq:ref_elem_to_eul}
    f^{(e)}(t,\zvec) = f(t,\xvec^{(e)}(t,\zvec))
\end{equation}
Examples of these reference-element functions include those for density $\rho^{(e)} = \rho^{(e)}(t,\zvec)$, velocity $\uvec^{(e)} = \uvec^{(e)}(t,\zvec)$, and internal energy $e^{(e)} = e^{(e)}(t,\zvec)$. Using integration by substitution and then \cref{eq:ref_elem_to_eul} we show 
\begin{align}
    \int_{\Omega^{(e)}} f dV_x &= \int_{\Omega_z} f(t, \xvec^{(e)}(t,\zvec)) J^{(e)} \, dV_z \nonumber \\
    &= \int_{\Omega_z} f^{(e)} J^{(e)} \, dV_z.
\end{align}
...
\begin{equation}
    \left ( \sigmavec \cdot \nabla f \right )_{\xvec=\xvec^{(e)}}= \left ( \sigma_{ij} \frac{\partial f}{\partial x_j} \right )_{\xvec = \xvec^{(e)}}= \sigma_{ij}^{(e)} \left ( \frac{\partial f}{\partial x_j} \right )_{\xvec = \xvec^{(e)}}.
\end{equation}
We first note that
\begin{equation}
    \frac{\partial f^{(e)}}{\partial z_k} = \left ( \frac{\partial f}{\partial x_i} \right )_{\xvec = \xvec^{(e)}} \frac{\partial x^{(e)}_i}{\partial z_k} = \left ( \frac{\partial f}{\partial x_i} \right )_{\xvec = \xvec^{(e)}} J^{(e)}_{ik}.
\end{equation}
Upon multiplying both sides by the inverse of $\Jvec^{(e)}$, we get
\begin{equation}
    \left ( \frac{\partial f}{\partial x_j} \right )_{\xvec = \xvec^{(e)}} = \frac{\partial f^{(e)}}{\partial z_k} \left( J^{(e)} \right)^{-1}_{kj}.
\end{equation}
Thus, we now have
\begin{equation}
    \left ( \sigmavec \cdot \nabla f \right )_{\xvec=\xvec^{(e)}} = \sigma^{(e)}_{ij}\frac{\partial f^{(e)}}{\partial z_k} \left( J^{(e)} \right)^{-1}_{kj} = \sigma^{(e)}_{ij} \left[ \left( J^{(e)} \right)^{-1} \right]^T_{jk} \frac{\partial f^{(e)}}{\partial z_k}.
\end{equation}
In tensor notation, the above is written as
\begin{equation}
    \left ( \sigmavec \cdot \nabla f \right )_{\xvec=\xvec^{(e)}} = \sigmavec^{(e)} \cdot \left[ \left( \Jvec^{(e)} \right)^{-1} \right]^T \cdot \nabla_\zvec f^{(e)}.
\end{equation}
Thus, for the force matrix $\fvec_{ij}$ we can now write
\begin{align}
    \int_{\Omega^{(e)}} \sigmavec \cdot \nabla w_i \phi_j \, dV_x &= \int_{\Omega_\zvec} \left ( \sigmavec \cdot \nabla w_i \phi_j \right )_{\xvec=\xvec^{(e)}} J^{(e)} \, dV_z \nonumber \\
    &= \int_{\Omega_\zvec} \sigmavec^{(e)} \cdot \left[ \left( \Jvec^{(e)} \right)^{-1} \right]^T  \cdot \nabla_\zvec w^{(e)}_i \phi^{(e)}_j J^{(e)} \, dV_z.
\end{align}

Finally, we note that we can evaluate \cref{eq:evol_rho_semi_discrete} at $\yvec = \xvec^{(e)}(0,\zvec)$ to obtain
\begin{equation}
