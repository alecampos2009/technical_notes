...
\begin{figure}[ht]
    ...
    \caption{Schematic of the three domains $\Omega_z$, $\Omega^e$, $\Omega^e_0$.}
    \label{fig:three_spaces}
 \end{figure}
We introduce the reference element as the unit square in 2D or the unit cube in 3D. The domain of this reference element is labelled as $\Omega_z$ and it doesn't change with time. We introduce the function $\xvec^e = \xvec^e(t,\zvec)$, which maps from points $\zvec$ in $\Omega_z$ to points in the finite element $e$ of the mesh. The domain of the finite element $e$ is labelled as $\Omega^e = \Omega^e(t,\Omega_z)$. We also introduce $\Omega^e_0$ as the initial domain of the finite element $e$, that is, $\Omega^e_0 = \Omega^e(0,\Omega_z)$. A depiction of these domains and their mappings is shown in \cref{fig:three_spaces}. $\xvec^e$ is related to $\xvec^+$ as follows
\begin{equation}
...
\begin{align}
    ...
    &= \left ( \Jvec^+ \right )_{\yvec = \xvec^e(0,\zvec)} \Jvec^e_0,
\end{align}
where $\Jvec^e_0 = \Jvec^e(0,\zvec)$.
Taking the determinant of the above gives
\begin{equation}
    \label{eq:ref_elem_det_J}
    J^e = \left ( J^+ \right )_{\yvec = \xvec^e(0,\zvec)} J^e_0,
\end{equation}
where $J^e_0 = J^e(0,\zvec)$.
