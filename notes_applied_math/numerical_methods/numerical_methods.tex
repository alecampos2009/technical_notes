...
%--------------------------------------------
Consider the boundary value problem in which a scalar $u$ is sought as the solution to the following
\begin{equation}
    \begin{aligned}
        &Au = f \qquad &&\text{in } \Omega \\
        &u = 0 && \text{on } \Gamma,
    \end{aligned}
\end{equation}
where $A$ is any given linear operator. The weak formulation of the above is to find the weak solution $u\in H^1_0$ such that 
\begin{equation}
    (Au,v) = (f,v) \qquad \forall v \in H^1_0,
\end{equation}
...
\begin{equation}
\label{eq:weak_form}
    a(u,v) = L(v) \qquad \forall v \in H^1_0 .
\end{equation}
...
\end{equation}
Label the finite basis of $S_h$ as $\{\Phi_i\}_{i=1}^n$, where $n$ is the total number of basis elements (also referred to as degrees of freedom). That is, any $v_h \in S_h$ can be written as $v_h = \sum_{i=1}^n V_i \Phi_i$. \Cref{eq:fe_elliptic_sh_solution} is then automatically satisfied if the following is satisfied
\begin{equation}
    a(u_h, \Phi_i) = (f, \Phi_i) \qquad \text{for }i=1,...,n.
\end{equation}

Since $u_h \in S_h$, we can write $u_h(x) = \sum_{j=1}^n U_j \Phi_j(x)$. Thus, the above becomes
\begin{equation}
    \sum_{j=1}^n U_j a(\Phi_j,\Phi_i) = (f,\Phi_i) \qquad \text{for }i=1,...,n.
\end{equation}
Define the vectors $\Uvec$ and $\bvec$, as those whose components are $U_j$ and $(f,\Phi_i)$, respectively. Define the matrix $\Avec$ as that whose components are $a(\Phi_j,\Phi_i)$, where $i$ denotes the row and $j$ the column. Then, the equation above can be written as
\begin{equation}
    \Avec \Uvec = \bvec.
\end{equation}
