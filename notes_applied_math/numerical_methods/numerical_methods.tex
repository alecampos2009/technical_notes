...
\end{equation}
where $f_f(t,u)$ is the fast source and $f_s(t,u)$ the slow source. Also introduce fast and slow functions $u_f=u_f(t)$ and $u_s=u_s(t)$ that satisfy the following two expressions
\begin{align}
    \frac{du_f}{dt} = f_f(t, u), \nonumber \\
    \frac{du_s}{dt}= f_s(t, u).
\end{align}
Plugging in the above two in \cref{eq:ode_generic_split} leads to $u = u_f + u_s$. For the sake of clarity, we'll explicitly denote the fast and slow functional arguments in the sources. Thus, the equations are written as
\begin{align}
...
\begin{itemize}
    ...
    \item Solve for $u^n_s$ using a non-linear solver to solve the implicit trapezoidal equation below
    \begin{equation}
