...
%----------------------------------------------------------------------------------------------------------------------------
%---------------------------------------------------------------
\section{Definition of the finite element method}
%---------------------------------------------------------------
%--------------------------------------------
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where $A$ is any given linear operator, $\Omega$ is the spatial domain, $\Gamma$ its boundary, and $\Rvec_+$ the set of positive real numbers. The weak formulation of the above is to find the weak solution $u\in H^1_0$ such that 
\begin{equation}
    \left ( \frac{\partial u}{\partial t} , v \right ) + (Au,v) = (f,v) \qquad \forall v \in H^1_0,
\end{equation}
where $(a,b)= \int_\Omega av \, d\xvec$ is the $L_2$ inner product. Using Calculus identities, some of the derivatives in $(Au,v)$ can be moved from operating on $u$ to operating on $v$. We label the end result as $a(u,v)$, which is a bilinear form. We also label $(f,v)$ as $L(v)$, which is a linear form. Thus, the above is rewritten as 
\begin{equation}
\label{eq:weak_form}
    \left ( \frac{\partial u}{\partial t} , v \right ) + a(u,v) = L(v) \qquad \forall v \in H^1_0 .
\end{equation}
...
%--------------------------------------------
The following step is to partition the domain into $M$ grid elements. Introduce $S_h \subset H^1_0$ as a finite dimensional space, that is, a space with a finite number of basis elements. We now aim to find $u_h \in S_h$ such that
\begin{equation}
    \label{eq:fe_elliptic_sh_solution}
        \left ( \frac{\partial u_h}{\partial t} , v_h \right ) + a(u_h,v_h) = (f,v_h)\qquad \forall v_h \in S_h.
    \end{equation}
Label the finite basis of $S_h$ as $\{\Phi_i\}_{i=1}^n$, where $n$ is the total number of basis elements (also referred to as degrees of freedom), and $\Phi_i = \Phi_i(\xvec)$. That is, any $v_h \in S_h$ can be written as $v_h = \sum_{i=1}^n V_i \Phi_i$, where $V_i = V_i(t)$. \Cref{eq:fe_elliptic_sh_solution} is then automatically satisfied if the following is satisfied
\begin{equation}
    \left ( \frac{\partial u_h}{\partial t}, \Phi_i \right ) + a(u_h, \Phi_i) = (f, \Phi_i) \qquad \text{for }i=1,...,n.
\end{equation}

Since $u_h \in S_h$, we can write $u_h(x) = \sum_{j=1}^n U_j \Phi_j$, where $U_j = U_j(t)$. Thus, the above becomes
\begin{equation}
    \sum_{j=1}^n (\Phi_j, \Phi_i) \frac{dU_j}{dt} + \sum_{j=1}^n U_j a(\Phi_j,\Phi_i) = (f,\Phi_i) \qquad \text{for }i=1,...,n.
\end{equation}
Define the vectors $\Uvec$ and $\bvec$, as those whose components are $U_j$ and $(f,\Phi_i)$, respectively. Define the matrix $\Avec$ as that whose components are $a(\Phi_j,\Phi_i)$, where $i$ denotes the row and $j$ the column. Similarily, define the mass matrix $\Mvec$ as that whose components are $(\Phi_j, \Phi_i)$, where $i$ denotes the row and $j$ the column. Then, the equation above can be written as
\begin{equation}
