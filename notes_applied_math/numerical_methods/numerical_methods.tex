...
\begin{equation}
    ...
    \mathcal{S}_h = \{v \in C \left ( \bar{\Omega} \right ) : v(K_j) \in \Pi_{k}(K_j) \text{ for } j=1,...,m, \text{ and } v = 0 \text{ on } \partial \Omega \times \Rvec_+ \}.
\end{equation}
In the above, $\Pi_{k}(K_j)$ denotes the space of polynomials of degree $\le k$ defined within the finite element $K_j$. In other words, $\mathcal{S}_h$ is the space of piecewise-polynomial continuous functions. See for example \cref{fig:fe_cg_sample_function}. for a pictorial representation of these functions. For finite $k$, $S_h$ is a finite dimensional space: any function in $S_h$ can be constructed as a linear combination of basis polynomial functions.
...
\begin{equation}
    [[u]] = u^+ - u^-,
\end{equation}
where $u^+$ is the value of $u$ on one side of the element boundary and $u^-$ is the value on the other side. Additionally, introduce $\Gamma_h$ as the skeleton of $\tau_h$, that is, the inner boundaries of all the finite elements in the domain. Thus, we now can write
\begin{equation}
    \int_\Omega \nabla \cdot (\lambdavec u) \, dV = \int_{\partial \Omega} u \lambdavec \cdot \nvec \, dS + \int_{\Gamma_h} [[u]]\lambdavec \cdot \nvec_K \, dS.
\end{equation}
...
\begin{equation}
    ...
    \int_\Omega \nabla \cdot (\lambdavec uv) \, d\xvec = \int_{\partial \Omega} uv \lambdavec \cdot \nvec \, dS + \int_{\Gamma_h} \left ( \{u\} [[v]] + [[u]] \{v\} \right )\lambdavec \cdot \nvec_K \, dS.
\end{equation}

Consider the derivation carried out to obtain \cref{eq:fe_sample_bilinear_form}. If both $u$ and $v$ are discontinuous, then \cref{eq:div_theorem_2funcs} is used rather than the traditional divergence theorem and thus the result becomes
\begin{equation}
    (\nabla \cdot(\boldsymbol{\lambda}u),v) = -(u\boldsymbol{\lambda},\nabla v) + \int_{\Gamma_h} \left ( \{u\} [[v]] + [[u]] \{v\} \right )\lambdavec \cdot \nvec_K \, dS.
\end{equation}
Thus, for DG, an additional term appears for every bilinear form. For example, \cref{eq:weak_form} for the weak formulation of the problem is now
\begin{equation}
        \left ( \frac{\partial u}{\partial t} , v \right ) + a(u,v)  + \int_{\Gamma_h} \left ( \{u\} [[v]] + [[u]] \{v\} \right )\lambdavec \cdot \nvec_K \, dS = L(v) \qquad \forall v \in \mathcal{V} .
\end{equation}
Unlike the finite dimensional space shown in \cref{eq:fe_cg_discrete_space}, we now use the following for DG 
\begin{equation}
    \mathcal{S}_h = \{v : v(K_j) \in \Pi_{k}(K_j) \text{ for } j=1,...,m \},
\end{equation}
where $\Pi_{k}(K_j)$ can be the same space as in the continuous Galerkin case. The main difference, however, is that the functions $v \in S_h$ can now be discontinuous from element to element (they do not belong to $C(\bar{\Omega})$). See \cref{fig:fe_dg_sample_function} for a pictorial representation of these functions.
