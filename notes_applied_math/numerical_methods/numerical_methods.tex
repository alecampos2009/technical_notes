...
\section{Multi-Rate}
%----------------------------------------------------------------------------------------------------------------------------
Split $u$ into fast and slow parts, that is $u = u_f + u_s$, where $u_f$ is defined by
\begin{equation}
    \frac{du_f}{dt} = f_f(t, u_f, u_s),
\end{equation}
and $u^s$ is defined by
\begin{equation}
    \frac{du_s}{dt}= f_s(t, u_f, u_s).
\end{equation}
The right-hand side of \cref{ode} is thus given by $f(t,u) = f_f(t,u_f, u_s)$ + $f_s(t,u_f,u_s)$. We solve this system using an implicit-trapezoidal multi-rate Runge-Kutta scheme.
