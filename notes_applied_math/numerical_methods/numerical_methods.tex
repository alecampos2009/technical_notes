...
\item $\{\Phi_i\}_{i=1}^{M-1}$ is the basis for $S_h$, where $M-1$ is the total number of interior points. That is, any $v_h \in S_h$ can be written as $v_h = \sum_{i=1}^{M-1} V_i \Phi_i$. \Cref{eq:fe_elliptic_sh_solution} is then automatically satisfied if the following is satisfied
\begin{equation}
