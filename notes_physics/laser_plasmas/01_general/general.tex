...
\end{equation*}
Summing over all species and using \cref{eq:coll_cons_momentum} leads to
\begin{equation}
...
Roughly speaking, Coulomb interactions are those which occur when the electric fields of two charge particles that are heading towards each other affect the behavior of those particles. We can define two types of Coulomb interactions:
\begin{itemize}
    \item Weak Coulomb interactions
    \begin{itemize}
        \item Particle kinetic energy is larger than Coulomb potential energy.
        \item Occur when the inter-particle distances are large, i.e.\@ \textit{long-range} interactions.
    \end{itemize}

    \item Strong Coulomb interactions
    \begin{itemize}
        \item Particle kinetic energy is smaller than Coulomb potential energy.
        \item Occur when the inter-particle distances are small, i.e.\@ \textit{short-range} interactions.
    \end{itemize}
\end{itemize}
We can also define two types of plasma regimes:
\begin{itemize}
    \item Weakly-coupled plasmas
    \begin{itemize}
        \item Weak interactions play a larger role than strong interactions.
        \item Exhibit collective behavior: caused by the electromagnetic fields that result from the integrated effect of a large number of distant particles. 
    \end{itemize}  

    \item Strongly-coupled plasmas
    \begin{itemize}
        \item Weak interactions play a smaller role than strong interactions.
        \item Do not exhibit collective behavior.
    \end{itemize} 
\end{itemize}
Collisionless plasmas are the extreme limit of weakly-coupled plasmas where only weak interactions play a role.

We describe an approximate Coulomb potential energy for particles in a plasma as
\begin{equation}
    U =  \frac{q_s^2}{4 \pi \epsilon_0 a_s}.
\end{equation}
The impact parameter that has been used above is $a_s$, the sphere radius. This radius provides a decent measure on the average spacing between particles in a plasma. Since the volume of a single particle is $1/n_s$, and if we assume that this volume is given by $4/3 \pi a_s^3$, then equating these two gives the expression for the sphere radius
\begin{equation}
