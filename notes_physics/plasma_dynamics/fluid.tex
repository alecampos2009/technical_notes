...
%--------------------------------------------
We'll make use of the density and momentum multi-fluid conservation laws described in \cref{sec:two_fluid_equations}. Assuming no shear stresses and collisions, we have for particle density and momentum
\begin{equation}
    \frac{\partial n_i}{\partial t} + \nabla \cdot \left (n_i \uvec_i \right ) = 0,
\end{equation}
\begin{equation}
...
\end{equation}
We'll also assume homentropic flow. As shown in my fluid equations, this means the ion and electron pressures satisfy
\begin{equation}
    p_i = C_i n_i^{\gamma_i},
\end{equation}
\begin{equation}
    p_e = C_e n_e^{\gamma_e},
\end{equation}
where $C_i$ and $C_e$ are constants across space and time. The fluid equations above will be complemented with the Maxwell equations shown in \cref{sec:two_fluid_equations}.

%--------------------------------------------
\section{Linearization}
%--------------------------------------------
\label{sec:p_waves_linearization}
The following decompositions will be used in some of the subsequent sections
\begin{align}
    n_i &= n_{i0} + n_{i1}, \nonumber \\
    n_e &= n_{e0} + n_{e1}, \nonumber \\
    p_i &= p_{i0} + p_{i1}, \nonumber \\
...
\begin{itemize}
    \item Terms with a subscript 1 are small and thus products of two small quantities can be neglected. 
    \item $\uvec_{i0}$, $\uvec_{e0}$, $\Evec_0$, and $\Bvec_0$ are zero.
    \item $n_{i0}$, $n_{e0}$, $p_{i0}$, and $p_{e0}$ are time independent.
\end{itemize}

Using the decompositions above in the electron density equation, we have
\begin{equation*}
...
\end{equation}
Using the decompositions above in the ion density equation, we have
\begin{equation*}
...
Using the decompositions above in the electron momentum equation, we have 
\begin{multline*}
...
\end{equation}
Using the decompositions above in the ion momentum equation, we have 
\begin{multline*}
...
We'll often need to take the gradient of the ion and electron pressure. As an example, consider the electron pressure,
\begin{equation*}
    \nabla p_e = C_e \gamma_e n_e^{\gamma_e-1} \nabla n_e = C_e \gamma_e \frac{n_e^{\gamma_e}}{n_e} \nabla n_e = \gamma_e \frac{p_e}{n_e} \nabla n_e,
\end{equation*}
which we re-write as 
\begin{equation}
    n_e \nabla p_e = \gamma_e p_e \nabla n_e.
\end{equation}
Using the decompositions, we have
\begin{equation}
    \left ( n_{e0} + n_{e1} \right ) \nabla \left ( p_{e0} + p_{e1} \right ) = \gamma_e \left ( p_{e0} + p_{e1} \right ) \nabla \left ( n_{e0} + n_{e1} \right ).
\end{equation}
Linearizing, we obtain
\begin{equation}
    \label{eq:p_waves_e_pressure_linearized}
    n_{e0} \nabla p_{e1} + n_{e1} \nabla p_{e0} = \gamma_e p_{e0} \nabla n_{e1} + \gamma_e p_{e1} \nabla n_{e0}.
\end{equation}
The same applies for the ions, and thus
\begin{equation}
    \label{eq:p_waves_i_pressure_linearized}
    n_{i0} \nabla p_{i1} + n_{i1} \nabla p_{i0} = \gamma_i p_{i0} \nabla n_{i1} + \gamma_i p_{i1} \nabla n_{i0}.
\end{equation}
...
\begin{itemize}
    ...
    \item No changes in magnetic field.
\end{itemize}

\Cref{eq:p_waves_e_mom_linearized} for the electron momentum now becomes
\begin{equation}
    \label{eq:ep_waves_mom_linearized_pressure}
    \frac{\partial m_e n_{e0} \uvec_{e1}}{\partial t} + e n_{e0} \Evec_1 = - \nabla p_{e1}.
\end{equation}
Given that we are assuming $n_{e0}$ and $p_{e0}$ are constant, \cref{eq:p_waves_e_pressure_linearized} becomes
\begin{equation}
    n_{e0} \nabla p_{e1} = \gamma_e p_{e0} \nabla n_{e1}.
\end{equation}
Using this in \cref{eq:ep_waves_mom_linearized_pressure},
\begin{equation}
    \label{eq:ep_waves_mom_linearized}
    \frac{\partial m_e n_{e0} \uvec_{e1}}{\partial t} + e n_{e0} \Evec_1 = - \frac{\gamma_e p_{e0}}{n_{e0}} \nabla n_{e1}.
\end{equation}
...
\Cref{eq:p_waves_i_mom_linearized} for the ion momentum now becomes
\begin{equation}
    \label{eq:ia_waves_mom_linearized_pressure}
    \frac{\partial m_i n_{i0} \uvec_{i1}}{\partial t} - Z e n_{i0} \Evec_1 = - \nabla p_{i1}.
\end{equation}
Given that we are assuming $n_{i0}$ and $p_{i0}$ are constant, \cref{eq:p_waves_i_pressure_linearized} becomes
\begin{equation}
    n_{i0} \nabla p_{i1} = \gamma_i p_{i0} \nabla n_{i1}.
\end{equation}
Using this in \cref{eq:ia_waves_mom_linearized_pressure},
\begin{equation}
    \label{eq:ia_waves_mom_linearized}
    \frac{\partial m_i n_{i0} \uvec_{i1}}{\partial t} - Z e n_{i0} \Evec_1 = - \frac{\gamma_i p_{i0}}{n_{i0}} \nabla n_{i1}.
\end{equation}
...
\end{equation}
For this case, we now assume that the mass of the electron, which is significantly smaller than that of the ions, is negligible. Thus, \cref{eq:p_waves_e_mom_linearized} simplifies to 
\begin{equation}
    \label{eq:ia_waves_E}
...
\begin{align}
    \nabla \times \Fvec_l &= 0, \label{eq:em_plasma_long_def}\\
    \nabla \cdot \Fvec_t &= 0. \label{eq:em_plasma_tran_def}
\end{align}
...
\end{equation}
\Cref{eq:em_plasma_long_def,eq:em_plasma_tran_def} now translate to
\begin{align}
    \kvec \times \hat{\Fvec}_l &= 0 \\
    \kvec \cdot \hat{\Fvec}_t &= 0
\end{align}
The first says $\hat{\Fvec}_l$ is parallel to $\kvec$ and the second says $\hat{\Fvec}_t$ is orthogonal to $\kvec$. Thus, $\hat{\Fvec}_l \cdot \hat{\Fvec}_t = 0$. We will often have situations where $\nabla \times \Fvec = \nabla \cdot \Fvec = 0$, which by its own does not imply $\Fvec = 0$. However, due to the wave form of $\Fvec$ (\cref{eq:em_general_wave_form}), we have $\kvec \times \hat{\Fvec} = \kvec \cdot \hat{\Fvec} =  0$. This can only be satisfied by $\hat{\Fvec} = 0$, and as a result we have $\Fvec = 0$. In other words,
\begin{equation}
    \label{eq:em_general_null_vector}
...
\begin{equation}
    \nabla \times \Evec = \nabla \times \Evec_t = 0.
\end{equation}
By definition, $\nabla \cdot \Evec_t = 0$. Thus, using \cref{eq:em_general_null_vector} we get $\Evec_t = 0$, that is, $\Evec = \Evec_l$. 

For electron-plasma waves, we can write \cref{eq:ep_waves_mom_linearized_pressure} in spectral to obtain
\begin{equation}
-i w m_e n_{e0} \hat{\uvec}_{e1} + e n_{e0} \hat{\Evec}_{1,l} = -\kvec \hat{p}_{e1}. 
\end{equation}
Since the second term on the left-hand side and the term on the right-hand side point along $\kvec$, $\hat{\uvec}_{e1}$ also points along $\kvec$, that is, $\hat{\uvec}_{e1} = \hat{\uvec}_{e1,l}$.

For ion-acoustic waves, we can write \cref{eq:ia_waves_mom_linearized_pressure} in spectral to obtain
\begin{equation}
-i w m_i n_{i0} \hat{\uvec}_{i1} - Z e n_{i0} \hat{\Evec}_{1,l} = -\kvec \hat{p}_{i1}. 
\end{equation}
Since the second term on the left-hand side and the term on the right-hand side point along $\kvec$, $\hat{\uvec}_{i1}$ also points along $\kvec$, that is, $\hat{\uvec}_{i1} = \hat{\uvec}_{i1,l}$. Finally, we note that the electric field being purely longitudinal is in agreement with \cref{eq:ia_waves_E}.
...
\begin{equation}
    \label{eq:em_general_E_potential}
    \Evec = -\nabla \phi - \frac{\partial \Avec}{\partial t},
\end{equation}
\begin{equation}
    \label{eq:em_general_B_potential}
    \Bvec = \nabla \times \Avec.
\end{equation}
...
\begin{equation*}
    \nabla \cdot \Bvec = \nabla \cdot \Bvec_l + \nabla \cdot \Bvec_t = \nabla \cdot \Bvec_l = 0.
\end{equation*}
However, by definition, $\nabla \times \Bvec_l = 0$ as well. Thus, using \cref{eq:em_general_null_vector}, we have $\Bvec_l = 0$. The same argument applies to the vector potential, and thus $\Avec_l = 0$. For the electric field, we have
\begin{equation*}
    \nabla \cdot \Evec = \nabla \cdot \Evec_l + \nabla \cdot \Evec_t = \nabla \cdot \Evec_l = \nabla \cdot \left ( -\nabla \phi \right ),
\end{equation*}
where we used \cref{eq:em_general_E_potential} for the last equality. In other words, we have
\begin{equation*}
...
\end{equation*}
Using \cref{eq:em_general_null_vector}, we conclude
\begin{equation}
    \label{eq:em_general_longitudinal_J}
...
\end{equation*}
Expanding the current density as $\Jvec = \Jvec_l + \Jvec_t$, and using \cref{eq:em_general_longitudinal_J}, we get
\begin{equation}
    \label{eq:em_general_trans_vec_pot}
    \frac{\partial^2 \Avec_t}{\partial t} - \frac{1}{\mu_0 \epsilon_0} \nabla^2 \Avec_t = \frac{1}{\epsilon_0} \Jvec_t.
\end{equation}
Applying the decomposition shown in \cref{eq:em_general_wave_form} to $\Avec_t$ and $\Jvec_t$ gives
\begin{equation}
    -w^2 \hat{\Avec}_t + \frac{k^2}{\mu_0 \epsilon_0} \hat{\Avec}_t= \frac{1}{\epsilon_0} \hat{\Jvec}_t.
\end{equation}
That is, $\Avec_t$ and $\Jvec_t$ point in the same direction.

Taking the time derivative of \cref{eq:em_general_trans_vec_pot} gives the equation for $\Evec_t$, that is
\begin{equation}
    \label{eq:em_general_trans_E}
    \frac{\partial^2 \Evec_t}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Evec_t + \frac{1}{\epsilon_0} \frac{\partial \Jvec_t}{\partial t} = 0.
\end{equation}
Taking the curls of \cref{eq:em_general_trans_vec_pot} gives the equation for $\Bvec_t$, that is
\begin{equation}
    \label{eq:em_general_trans_B}
    \frac{\partial^2 \Bvec_t}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Bvec_t - \frac{1}{\epsilon_0} \nabla \times \Jvec_t = 0.
\end{equation}
Applying the decomposition shown in \cref{eq:em_general_wave_form} to $\Evec_t$, $\Bvec_t$ and $\Jvec_t$ gives
\begin{equation}
    -w^2 \hat{\Evec}_t + \frac{k^2}{\epsilon_0 \mu_0} \hat{\Evec}_t - \frac{iw}{\epsilon_0} \hat{\Jvec}_t = 0,
\end{equation}
\begin{equation}
    -w^2 \Bvec_t + \frac{k^2}{\epsilon_0 \mu_0} \Bvec_t - \frac{i}{\epsilon_0} \kvec \times \Jvec_t = 0.
\end{equation}
That is, $\Evec_t$ points in the same direction as $\Jvec_t$, which as shown before points in the same direction as $\Avec_t$. Additionally, $\Bvec_t$ points in the direction of $\kvec \times \Jvec_t$, that is, it is orthogonal to $\Evec_t$.

It will often be the case that transverse waves will oscillate at too fast of a rate for the ions to react to the transverse effects, due to their large inertia. Thus, we can assume $\uvec_{i,t} = 0$. Given the definition of the current density in \cref{eq:twof_curr_density}, the transverse current density is given by $\Jvec_t = e \left (Z n_i \uvec_{i,t} - n_e \uvec_{e,t} \right )$, which simplifies to $\Jvec_t = -e n_e \uvec_{e,t}$.

%--------------------------------------------
\section{Transverse electromagnetic waves in plasmas}
%--------------------------------------------
...
\begin{itemize}
    ...
    \item Due to larger inertia, ion motion does not react fast enough to fields, $\uvec_{i1} = 0$.
\end{itemize}
Given the assumptions above, $\rho_e = e(Zn_i - n_e)$ becomes $\rho_e = 0$, whereas $\Jvec = e \left (Z n_i \uvec_i - n_e \uvec_e \right )$ simplifies to $\Jvec = -e n_{e0} \uvec_{e1}$. We also note that $\Jvec_t = -e n_{e0} \uvec_{e1,t}$.

Since the $\Evec_0 = \Bvec_0 = 0$, we re-write \cref{eq:em_general_trans_E,eq:em_general_trans_B} as
\begin{equation}
...
\begin{equation}
    ...
    \frac{\partial m_e n_{e0} \uvec_{e1}}{\partial t} + e n_{e0} \Evec_1 = - \nabla p_{e0}.
\end{equation}
We'll assume $\Avec_t \cdot \nabla n_{e0} = 0$. Since the direction of $\nabla n_{e0}$ is the same as the direction of $\nabla p_{e0}$, then $\nabla p_{e0}$ is orthogonal to $\Avec_t$, $\Evec_t$ and $\Jvec_t$. Thus, the transverse component of the above gives
\begin{equation}
