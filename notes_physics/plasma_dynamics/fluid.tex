...
\end{equation}
These are the so-called plane waves. Note that $\hat{\Fvec}$, $\hat{\Fvec}_l$, and $\hat{\Fvec}_t$ are complex vectors also, where the real and complex components point in the same direction. Also note that different waves will wave different wave vectors. For example, we'll use $\kvec_e$ for electron-plasma waves, $\kvec_i$ for ion-acoustic waves, $\kvec_L$ for laser waves and $\kvec_s$ for scattered waves.
...
For electron-plasma waves, we can write \cref{eq:ep_waves_mom_linearized_pressure} in spectral form to obtain
\begin{equation}
-i w m_e n_{e0} \hat{\uvec}_{e1} + e n_{e0} \hat{\Evec}_{1,l} = -\kvec_e \hat{p}_{e1}. 
\end{equation}
...
\begin{equation}
-i w m_i n_{i0} \hat{\uvec}_{i1} - Z e n_{i0} \hat{\Evec}_{1,l} = -\kvec_i \hat{p}_{i1}. 
\end{equation}
...
or
\begin{equation*}
    \frac{\partial^2 \Avec_t}{\partial t} - c_0^2 \nabla^2 \Avec_t = \frac{1}{\epsilon_0} \Jvec - \frac{\partial \nabla \phi}{\partial t},
\end{equation*}
where $c_0 = 1/\sqrt{\mu_0 \epsilon_0}$. Expanding the current density as $\Jvec = \Jvec_l + \Jvec_t$, and using \cref{eq:em_general_longitudinal_J}, we get
\begin{equation}
    \label{eq:em_general_trans_vec_pot}
    \frac{\partial^2 \Avec_t}{\partial t} - c_0^2 \nabla^2 \Avec_t = \frac{1}{\epsilon_0} \Jvec_t.
\end{equation}
Using the functional form in \cref{eq:em_general_wave_form} for $\Avec_t$ and $\Jvec_t$ gives
\begin{equation}
    -w^2 \hat{\Avec}_t + k^2 c_0^2 \hat{\Avec}_t= \frac{1}{\epsilon_0} \hat{\Jvec}_t.
\end{equation}
...
\end{equation}
Taking the curl of \cref{eq:em_general_trans_vec_pot} gives the equation for $\Bvec_t$, that is
\begin{equation}
    \label{eq:em_general_trans_B}
    \frac{\partial^2 \Bvec_t}{\partial t^2} - c_0^2 \nabla^2 \Bvec_t - \frac{1}{\epsilon_0} \nabla \times \Jvec_t = 0.
\end{equation}
Using the functional form in \cref{eq:em_general_wave_form} for $\Evec_t$, $\Bvec_t$ and $\Jvec_t$ gives
\begin{equation}
    -w^2 \hat{\Evec}_t + k^2 c_0^2 \hat{\Evec}_t - \frac{iw}{\epsilon_0} \hat{\Jvec}_t = 0,
\end{equation}
\begin{equation}
    -w^2 \Bvec_t + k^2 c_0^2 \Bvec_t - \frac{i}{\epsilon_0} \kvec \times \Jvec_t = 0.
\end{equation}
...
It will often be the case that transverse waves will oscillate at such a fast rate that the ions, which have too large of an inertia, will be unable to react to the oscillations. Thus, we can assume $\uvec_{i,t} = 0$. Given the definition of the current density in \cref{eq:p_waves_curr_density}, the transverse current density is expressed as $\Jvec_t = e \left (Z n_i \uvec_{i,t} - n_e \uvec_{e,t} \right )$, which simplifies to 
\begin{equation}
    \label{eq:em_transverse_current}
    \Jvec_t = -e n_e \uvec_{e,t}.
\end{equation}
Thus, we now need an expression for $\uvec_{e,t}$. 
...
\begin{equation*}
    m_e n_e \frac{\partial\uvec_e}{\partial t} + m_e n_e \uvec_e \cdot \nabla \uvec_e + e n_e \left [ -\nabla \phi - \frac{\partial \Avec}{\partial t} + \uvec_e \times \left ( \nabla \times \Avec \right ) \right ] = -\nabla p_e.
\end{equation*}
Using the vector identity $\nabla \left ( F^2 / 2 \right ) = \Fvec \times \left ( \nabla \times \Fvec \right ) + \Fvec \cdot \nabla \Fvec$, we write the above as
\begin{equation*}
    m_e n_e \frac{\partial\uvec_e}{\partial t} - m_e n_e \uvec_e \times \left ( \nabla \times \uvec_e \right ) + m_e n_e \nabla \left (\frac{u_e^2}{2} \right ) + e n_e \left [ -\nabla \phi - \frac{\partial \Avec}{\partial t} + \uvec_e \times \left ( \nabla \times \Avec \right ) \right ] = -\nabla p_e.
\end{equation*}
We need the component of the above along the $\uvec_{e,t}$ direction, which is the same as the $\Jvec_t$, $\Evec_t$, and $\Avec_t$ directions. The requirement that this direction be orthogonal to $\kvec$ does not uniquely define it (there are multiple directions orthogonal to $\kvec$). Additionally, if there are multiple waves each with its own $\kvec$, which should be used?  We'll assume wave vectors of all waves (whether transverse or longitudinal) lie on a so-called base plane. That is, $\kvec_e$, $\kvec_i$, $\kvec_L$, and $\kvec_s$ are all on this plane. We now choose the direction of $\Jvec_t$, $\Evec_t$, $\Avec_t$ to be the direction orthogonal to this plane, so that these vectors are orthogonal to any $\kvec$. Note that since all wave vectors point along the base plane, $u_e^2$, $\phi$ and $p_e$ will only vary along that plane, and thus their gradients will be confined to that plane. Thus, the component of the electron momentum equation orthogonal to the base plane is
\begin{equation*}
    m_e n_e \frac{\partial\uvec_{e,t}}{\partial t} - m_e n_e \left [ \uvec_e \times \left ( \nabla \times \uvec_e \right ) \right ]_{\Avec_t} + e n_e \left \{ - \frac{\partial \Avec_t}{\partial t} + \left [ \uvec_e \times \left ( \nabla \times \Avec_t \right ) \right ]_{\Avec_t} \right \} = 0.
\end{equation*}
where the $\Avec_t$ subscript indicates component along the direction of $\Avec_t$. We'll re-write the above as
\begin{equation*}
    \frac{\partial\uvec_{e,t}}{\partial t} - \left [ \uvec_e \times \left ( \nabla \times \uvec_e \right ) \right ]_{\Avec_t} + \frac{e}{m_e} \left \{ - \frac{\partial \Avec_t}{\partial t} + \left [ \uvec_e \times \left ( \nabla \times \Avec_t \right ) \right ]_{\Avec_t} \right \} = 0.
\end{equation*}
Something something...
\begin{equation}
    \label{eq:em_transverse_velocity}
    \uvec_{e,t} = \frac{e\Avec_t}{m_e}.
\end{equation}

Using both the transverse current given by \cref{eq:em_transverse_current} and the transverse velocity given by \cref{eq:em_transverse_velocity} in \cref{eq:em_general_trans_vec_pot} gives
\begin{equation*}
    \frac{\partial^2 \Avec_t}{\partial t} - c_0^2 \nabla^2 \Avec_t = -\frac{e n_e}{\epsilon_0} \uvec_{e,t} = -\frac{e^2 n_e}{\epsilon_0 m_e} \Avec_t.
\end{equation*}
We now use the decomposition $n_e = n_{e0} + n_{e1}$, where $n_{e0}$ is time independent. The above becomes
\begin{equation}
    \label{eq:em_general_trans_vec_pot_complete}
    \frac{\partial^2 \Avec_t}{\partial t} + w_{pe}^2 \Avec_t - c_0^2 \nabla^2 \Avec_t = -\frac{e^2 n_{e1}}{\epsilon_0 m_e} \Avec_t,
\end{equation}
where $w_{pe}^2 = e^2 n_{e0} / m_e \epsilon_0$.
% Using the scaling $c \sim w/k$, we show the following 
% \begin{align}
%     \frac{1}{c^2} \frac{\partial \uvec_e}{\partial t} &= -\frac{iw \uvec_e}{c^2} \sim i \frac{\uvec_e}{c} k \nonumber \\
%     \frac{1}{c^2} \uvec_e \cdot \nabla \uvec_e & = \frac{i \left ( \uvec_e \cdot \kvec \right ) \uvec_e}{c^2} \sim i \frac{\uvec_e}{c} \frac{\uvec_e}{c} k \nonumber \\
%     \frac{1}{c^2} \frac{\partial \Avec}{\partial t} &= -\frac{iw \Avec}{c^2} \sim i \frac{\Avec}{c} k \nonumber \\
%     \frac{1}{c^2} \uvec_e \times \left ( \nabla \times \Avec \right ) &= \frac{i \uvec_e \times \left ( \kvec \times \Avec \right )}{c^2} \sim i \frac{\uvec_e}{c} \frac{\Avec}{c} k
% \end{align}

%--------------------------------------------
\section{Electromagnetic waves in a stable plasma}
%--------------------------------------------
We start with \cref{eq:em_general_trans_vec_pot_complete}, but focus on the stable-plasma case, that is, $n_{e1} = 0$. Thus, we have
\begin{equation}
    \label{eq:tem_general_trans_vec_pot_complete}
    \frac{\partial^2 \Avec_t}{\partial t} + w_{pe}^2 \Avec_t - c_0^2 \nabla^2 \Avec_t = 0.
\end{equation}
We note that $n_{e0}$ is only time independent, that is, it  still allowed to vary across space. As a result, $w_{pe}^2$ is also allowed to vary across space. We'll now use the functional form in \cref{eq:em_general_wave_form} for $\Avec_t$. However, we'll express the vector potential as $\Avec_t = \tilde{\Avec}_t \exp(-iwt)$, where $\tilde{\Avec}_t = \hat{\Avec}_t \exp(i\kvec \cdot \xvec)$. Thus, \cref{eq:tem_general_trans_vec_pot_complete} gives
\begin{equation}
    -w^2 \Avec_t + w_{pe}^2 \Avec_t - c_0^2 \nabla^2 \Avec_t = 0.
\end{equation}
We re-write the above as
\begin{equation}
    \frac{w^2}{c_0^2} \Avec_t - \frac{w^2}{c_0^2} \frac{w_{pe}^2}{w^2} \Avec_t + \nabla^2 \Avec_t = 0.
\end{equation}
Defining $\epsilon = 1 - w_{pe}^2 / w^2$, we ultimately get
\begin{equation}
    \label{eq:tem_general_trans_vec_pot_complete_spec}
    \frac{w^2}{c_0^2} \epsilon \Avec_t + \nabla^2 \Avec_t = 0.
\end{equation}

Taking the time derivative of \cref{eq:tem_general_trans_vec_pot_complete} gives the equation for $\Evec_t$, that is
\begin{equation}
    \label{eq:tem_general_trans_E_complete}
...
Taking the curl of \cref{eq:em_general_trans_vec_pot_complete} gives the equation for $\Bvec_t$, that is
\begin{equation}
    \label{eq:em_general_trans_B_complete}
    \frac{\partial^2 \Bvec_t}{\partial t^2} + w_{pe}^2 \Bvec_t - c_0^2 \nabla^2 \Bvec_t + \nabla w_{pe}^2 \times \Avec_t = 0.
\end{equation}
Using $\Bvec_t = \tilde{\Bvec}_t \exp(-iwt)$, where $\tilde{\Bvec}_t = \hat{\Bvec}_t \exp(i \kvec \cdot \xvec)$, we get
\begin{equation}
    -w^2 \Bvec_t + w_{pe}^2 \Bvec_t - c_0^2 \nabla^2 \Bvec_t + \nabla w_{pe}^2 \times \Avec_t = 0.
\end{equation}
We re-write the above as
\begin{equation}
    \frac{w^2}{c_0^2} \Bvec_t - \frac{w^2}{c_0^2} \frac{w_{pe}^2}{w^2} \Bvec_t + \nabla^2 \Bvec_t - \frac{w^2}{c_0^2} \nabla \left ( \frac{w_{pe}^2}{w^2} \right ) \times \Avec_t = 0,
\end{equation}
which becomes
\begin{equation}
    \frac{w^2}{c_0^2} \epsilon \Bvec_t + \nabla^2 \Bvec_t + \frac{w^2}{c_0^2} \nabla \epsilon \times \Avec_t = 0,
\end{equation}
Using \cref{eq:tem_general_trans_vec_pot_complete_spec} we get 
\begin{equation}
    \frac{w^2}{c_0^2} \epsilon \Bvec_t + \nabla^2 \Bvec_t - \frac{1}{\epsilon} \nabla \epsilon \times \nabla^2 \Avec_t = 0,
\end{equation}
The vector identity $\nabla \times \left ( \nabla \times \Fvec \right ) = \nabla \left ( \nabla \cdot \Fvec \right ) - \nabla^2 \Fvec$ gives $\nabla^2 \Avec_t = - \nabla \times \left ( \nabla \times \Avec_t \right ) = - \nabla \times \Bvec_t$. Thus, we finally get
\begin{equation}
    \frac{w^2}{c_0^2} \epsilon \Bvec_t + \nabla^2 \Bvec_t + \frac{1}{\epsilon} \nabla \epsilon \times \left ( \nabla \times \Bvec_t \right ) = 0.
\end{equation}
