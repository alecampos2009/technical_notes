...
\begin{equation}
    \label{eq:ep_waves_e_pressure_linearized}
...
\end{align}
Note that since all wave vectors point along the base plane, $u_e^2$, $\phi$ and $p_e$ will only vary along that plane, and thus their gradients will be confined to that plane. Thus, the component \cref{eq:emp_electron_momentum} along the main transverse direction is
\begin{equation}
...
%-------------------------------------------------------------------------------
We now drop the assumption that the plasma is stable, that is, we assume $n_{e1}$ is small but not zero. For simplicity, we'll also assume the plasma is uniform, that is $n_{e0}$ is not only constant in time but also in space. Thus, $p_{e0}$ is uniform in space and time, and $p_{e1}$ has small non-zero values. We'll also introduce the decomposition $\Avec = \Avec_L + \Avec_s$, where $\Avec_L$ is the vector potential associated with the laser light and $\Avec_s$ is the potential associated with the scattered light. For linearization purposes, we'll assume $\Avec_s$ is small. Thus, \cref{eq:emp_general_trans_vec_pot_complete} is written as 
\begin{equation}
    \label{eq:srs_temp1}
...
\end{equation}
Neglecting all terms with at least one small quantity, we get
\begin{equation}
    \label{eq:srs_temp2}
    \frac{\partial^2 \Avec_L}{\partial t} + w_{pe}^2 \Avec_L - c_0^2 \nabla^2 \Avec_L = 0,
\end{equation}
which is essentially \cref{eq:temp_general_trans_vec_pot_complete}, that is, the equation for an electromagnetic wave in a stable plasma. Neglecting all terms in \cref{eq:srs_temp1} that include products of two small quantities, and using \cref{eq:srs_temp2}, we get
\begin{equation}
    \frac{\partial^2 \Avec_s}{\partial t} + w_{pe}^2 \Avec_s - c_0^2 \nabla^2 \Avec_s = -\frac{e^2 n_{e1}}{\epsilon_0 m_e} \Avec_L.
\end{equation}
Thus, the fluctuating $n_{e1}$ couples with the laser light to serve as a source for the scattered light.

Consider now the electron momentum equation. Subtracting \cref{eq:emp_electron_momentum_transverse} from \cref{eq:emp_electron_momentum} gives
\begin{equation}
...
We now follow similar steps to those taken to derive the electron-plasma wave equation. Decompose the longitudinal electron velocity as $\uvec_{e,l} = \uvec_{e0,l} + \uvec_{e1,l}$, and assume $\uvec_{e0,l} = 0$ and $\uvec_{e1,l}$ is small. We'll also decompose the longitudinal electric field as $\Evec_l = \Evec_{0,l} + \Evec_{1,l}$, and assume $\Evec_{0,l} = 0$ and $\Evec_{0,1}$ is small. Linearizing the electron momentum equation then gives
\begin{equation*}
    \frac{\partial n_{e0} \uvec_{e1,l}}{\partial t} + n_{e0} \nabla \left (\frac{u_{e,t}^2}{2} \right ) + \frac{e n_{e0}}{m_e} \Evec_{1,l} = -\frac{1}{m_e} \nabla p_{e1}.
\end{equation*}
Using \cref{eq:ep_waves_e_pressure_linearized}, we get
\begin{equation}
    \label{eq:srs_mom_linearized}
    \frac{\partial n_{e0} \uvec_{e1,l}}{\partial t} + n_{e0} \nabla \left (\frac{u_{e,t}^2}{2} \right ) + \frac{e n_{e0}}{m_e} \Evec_{1,l} = -\frac{\gamma_e p_{e0}}{n_{e0} m_e} \nabla n_{e1}.
\end{equation}
The density equation is now written as
\begin{equation*}
...
\end{equation*}
Using the linearization assumptions, the above becomes
\begin{equation*}
    \frac{n_{e1}}{\partial t } + \uvec_{e,t} \cdot \nabla n_{e1} + \nabla \cdot \left ( n_{e0} \uvec_{e1,l} \right ) = 0.
\end{equation*}
Since $\uvec_{e,t}$ and $\nabla n_{e1}$ are orthogonal, we finally have
\begin{equation*}
    \frac{\partial n_{e1}}{\partial t} + \nabla \cdot \left ( n_{e0} \uvec_{e1,l} \right ) = 0.
\end{equation*}
Taking the derivative of the above and using \cref{eq:srs_mom_linearized} leads to the the wave equation for electron density
\begin{equation*}
    \frac{\partial^2 n_{e1}}{\partial t^2} - n_{e0} \nabla^2 \left ( \frac{ u_{e,t}^2}{2} \right ) - \frac{e n_{e0}}{m_e} \nabla \cdot \Evec_{1,l} = \frac{\gamma_e p_{e0}}{n_{e0} m_e} \nabla^2 n_{e1}.
\end{equation*}
As before, using \cref{eq:ep_waves_efield_divergence} we have
\begin{equation*}
    \frac{\partial^2 n_{e1}}{\partial t^2} - n_{e0} \nabla^2 \left ( \frac{ u_{e,t}^2}{2} \right ) + \frac{e^2 n_{e0}}{m_e \epsilon_0} n_{e1} = \frac{\gamma_e p_{e0}}{n_{e0} m_e} \nabla^2 n_{e1}.
\end{equation*}
or
\begin{equation}
    \frac{\partial^2 n_{e1}}{\partial t^2} + w_{pe}^2 n_{e1} - \frac{\gamma_e p_{e0}}{n_{e0} m_e} \nabla^2 n_{e1} =  n_{e0} \nabla^2 \left ( \frac{ u_{e,t}^2}{2} \right ).
\end{equation}
...
\end{equation}
We'll focus only on the effect of the cross term, and thus, we write the electron-plasma wave equation as
\begin{equation}
