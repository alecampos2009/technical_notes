...
%--------------------------------------------
The starting point are the density and momentum multi-fluid conservation laws in \cref{sec:two_fluid_equations}. Assuming no shear stresses and collisions, we have for particle density and momentum
\begin{equation}
    \frac{\partial n_i}{\partial t} + \nabla \cdot \left (n_i \uvec_i \right ) = 0,
\end{equation}
\begin{equation}
...
\end{equation}
We will now decompose the variables as follows 
\begin{align}
    n_i &= n_{i0} + n_{i1}, \nonumber \\
    n_e &= n_{e0} + n_{e1}, \nonumber \\
    p_i &= p_{i0} + p_{i1}, \nonumber \\
...
\begin{itemize}
    ...
    \item All terms with a subscript 1 are small and thus products of two small quantities can be neglected. 
    \item The flow is isentropic.
    \item Quasi-neutrality for the base flow: $Zn_{i0} = n_{e0}$. 
\end{itemize}
Finally, we'll assume the following for each of the two waves:
\begin{itemize}
    \item electron plasma wave: $n_{i1} = 0$.
    \item ion acoustic wave: $m_e \to 0$ and $Z n_{i1} \approx n_{e1}$.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{equation}
        \label{eq:electrodynamics_E_wave}
        \Evec = \sum_{\nvec} \tilde{\Evec}_{\nvec} e^{i \left ( \kvec_{\nvec} \cdot \xvec - wt \right )},
    \end{equation}
    and 
    \begin{equation}
        \label{eq:electrodynamics_B_wave}
        \Bvec = \sum_{\nvec} \tilde{\Bvec}_{\nvec} e^{i \left ( \kvec_{\nvec} \cdot \xvec - wt \right )}.
    \end{equation}
