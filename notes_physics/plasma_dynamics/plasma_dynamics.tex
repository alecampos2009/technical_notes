...
%-------------------------------------------------------------------------------
In this section we'll make heavy use of the density and momentum multi-fluid conservation laws described in \cref{sec:two_fluid_equations}. Assuming no shear stresses and collisions, we have for particle density and momentum
\begin{equation}
    \frac{\partial n_i}{\partial t} + \nabla \cdot \left (n_i \uvec_i \right ) = 0,
\end{equation}
\begin{equation}
...
We will also make ample use of the following decompositions
\begin{align}
    n_i &= n_{i0} + n_{i1}, \nonumber \\
    n_e &= n_{e0} + n_{e1}, \nonumber \\
    p_i &= p_{i0} + p_{i1}, \nonumber \\
    ...
    \Bvec &= \Bvec_0 + \Bvec_1.
\end{align}
In the above, all terms with a subscript 1 are small and thus products of two small quantities can be neglected.

%--------------------------------------------
\section{Electron Plasma Waves}
%--------------------------------------------
We'll use a few assumptions in the derivation of the equations:
\begin{itemize}
    \item The flow is homentropic.
    \item $n_{i0}$, $n_{e0}$, $p_{i0}$, and $p_{e0}$ are constant.
    \item $\uvec_{i0}$, $\uvec_{e0}$, $\Evec_0$, and $\Bvec_0$ are zero.
    \item Quasi-neutrality for the base flow, $Zn_{i0} = n_{e0}$.
    \item Uniform ion background, $n_{i1} = 0$. 
\end{itemize}

Given the decompositions in the previous section, we can write the electron density equation as
\begin{equation*}
    \frac{\partial n_{e0} + n_{e1}}{\partial t} + \nabla \cdot \left [ \left ( n_{e0} + n_{e1} \right )  \left ( \uvec_{e0} + \uvec_{e1} \right ) \right ] = 0,
\end{equation*}
which simplifies to
\begin{equation}
    \label{eq:ep_waves_den_linearized}
...
which simplifies to 
\begin{equation}
    \label{eq:ep_waves_mom_linearized_pressure}
    \frac{\partial m_e n_{e0} \uvec_{e1}}{\partial t} + e n_{e0} \Evec_1 = - \nabla p_{e1}.
\end{equation}
We now use the isentropic flow assumption. As is the case for inert fluids, any thermodynamic quantity will now depend on only one rather than two thermodynamic variables, that is, for $p_e = p_e(s_e, n_e)$ we now have 
\begin{equation}
    \left ( \frac{\partial p_e}{\partial s_e} \right )_{n_e} = 0. 
\end{equation}
A Taylor-series expansion gives
\begin{equation}
    p_e = p_{e0} + \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \left ( n_{e} - n_{e0} \right ) + h.o.t.
\end{equation}
Using the above in \cref{eq:ep_waves_mom_linearized_pressure},
\begin{equation}
    \label{eq:ep_waves_mom_linearized}
    \frac{\partial m_e n_{e0} \uvec_{e1}}{\partial t} + e n_{e0} \Evec_1 = - \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \nabla n_{e1}.
\end{equation}

Taking the time derivative of \cref{eq:ep_waves_den_linearized} and using \cref{eq:ep_waves_mom_linearized} leads to the the wave equation for electron density
\begin{equation}
    \frac{\partial^2 n_{e1}}{\partial t^2} - \frac{e n_{e0}}{m_e} \nabla \cdot \Evec_1 = \frac{1}{m_e} \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \nabla^2 n_{e1}.
\end{equation}
...
\end{equation}
We now use the quasi-neutrality assumption ($Zn_{i0} = n_{e0}$) and plug in the above in the electron wave equation to obtain
\begin{equation*}
    \frac{\partial^2 n_{e1}}{\partial t^2} + \frac{e^2 n_{e0}}{m_e \epsilon_0} n_{e1} =  \frac{1}{m_e} \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \nabla^2 n_{e1},
\end{equation*}
or 
\begin{equation}
    \frac{\partial^2 n_{e1}}{\partial t^2} + w^2_{pe} n_{e1} =  \frac{1}{m_e} \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \nabla^2 n_{e1}.
\end{equation}
Finally, as for ideal gasses, we have 
\begin{equation}
    \label{eq:electron_speed_sound}
    \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} = \gamma_e \left ( \frac{\partial p_e}{\partial n_e} \right )_{T_e} = \gamma_e k_B T_e = \frac{\gamma_e p_e}{n_e}.
\end{equation}
Thus, the wave equation for electron plasma waves is finally written as
\begin{equation}
    \frac{\partial^2 n_{e1}}{\partial t^2} + w^2_{pe} n_{e1} - \frac{\gamma_e p_{e0}}{n_{e0} m_e} \nabla^2 n_{e1} = 0.
\end{equation}

Assuming a mode of the form $n_{e1} = \hat{n}_{e1} \exp [ i \left ( \kvec \cdot \xvec - w t \right ) ]$ gives the following dispersion relation
\begin{equation}
    w^2 - w^2_{pe} - \frac{\gamma_e p_{e0}}{n_{e0} m_e}  k^2 = 0.
\end{equation}
If we define the thermal velocity without the factor of two, that is, $v_{T\alpha} = \sqrt{k_BT_\alpha / m_\alpha}$, then we have
\begin{equation}
    w^2 - w^2_{pe} - \gamma_e k^2 v^2_{T_{e0}} = 0.
\end{equation}
...
Unlike the case for the electron-plasma waves, we now do not assume that $n_i$ varies in space and time so slowly that it can be assumed constant. That is, we no longer assume $n_{i1} = 0$. Thus, the ion density equation gives
\begin{equation*}
    \frac{\partial n_{i0} + n_{i1}}{\partial t} + \nabla \cdot \left [ \left ( n_{i0} + n_{i1} \right )  \left ( \uvec_{i0} + \uvec_{i1} \right ) \right ] = 0,
\end{equation*}
which simplifies to
\begin{equation}
    \label{eq:ia_waves_den_linearized}
...
which simplifies to 
\begin{equation}
    \label{eq:ia_waves_mom_linearized_pressure}
    \frac{\partial m_i n_{i0} \uvec_{i1}}{\partial t} - Z e n_{i0} \Evec_1 = - \nabla p_{e1}.
\end{equation}
As with the electron equation, this can be written as
\begin{equation}
    \label{eq:ia_waves_mom_linearized}
    \frac{\partial m_i n_{i0} \uvec_{i1}}{\partial t} - Z e n_{i0} \Evec_1 = - \left [ \left ( \frac{\partial p_i}{\partial n_i} \right )_{s_i} \right ]_{n_i = n_{i0}} \nabla n_{i1}.
\end{equation}

Taking the time derivative of \cref{eq:ia_waves_den_linearized} and using \cref{eq:ia_waves_mom_linearized} leads to the the wave equation for ion density
\begin{equation}
    \frac{\partial^2 n_{i1}}{\partial t^2} + \frac{Z e n_{i0}}{m_i} \nabla \cdot \Evec_1 = \frac{1}{m_i} \left [ \left ( \frac{\partial p_i}{\partial n_i} \right )_{s_i} \right ]_{n_i = n_{i0}} \nabla^2 n_{i1}.
\end{equation}
For this case, we now assume that the mass of the electron, which is significantly smaller than that of the ions, is negligible. Thus, \cref{eq:ep_waves_mom_linearized} simplifies to 
\begin{equation}
    e n_{e0} \Evec_1 = - \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \nabla n_{e1}.
\end{equation}
Plugging in the above in the ion wave equation we obtain
\begin{equation}
    \frac{\partial^2 n_{i1}}{\partial t^2} = \frac{Z n_{i0}}{ n_{e0}} \frac{1}{m_i} \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} \nabla^2 n_{e1} + \frac{1}{m_i} \left [ \left ( \frac{\partial p_i}{\partial n_i} \right )_{s_i} \right ]_{n_i = n_{i0}} \nabla^2 n_{i1}.
\end{equation}
Due to quasi-neutrality, we have $Z n_{i0} = n_{e0}$ and $Z n_{i1} \approx n_{e1}$, which gives
\begin{equation}
    \frac{\partial^2 n_{i1}}{\partial t^2} = \left \{ \frac{Z}{m_i} \left [ \left ( \frac{\partial p_e}{\partial n_e} \right )_{s_e} \right ]_{n_e = n_{e0}} + \frac{1}{m_i} \left [ \left ( \frac{\partial p_i}{\partial n_i} \right )_{s_i} \right ]_{n_i = n_{i0}} \right \} \nabla^2 n_{i1}.
\end{equation}
The equivalent of \cref{eq:electron_speed_sound} for ions is
\begin{equation}
    \label{eq:ion_speed_sound}
    \left ( \frac{\partial p_i}{\partial n_i} \right )_{s_i} = \gamma_i \left ( \frac{\partial p_i}{\partial n_i} \right )_{T_i} = \gamma_i k_B T_i = \frac{\gamma_i p_i}{n_i}.
\end{equation}
Thus, the wave equation for ion acoustic waves is finally written as
\begin{equation}
    \frac{\partial^2 n_{i1}}{\partial t^2} - \left ( \frac{Z \gamma_e k_B T_{e0} + \gamma_i k_B T_{i0}}{m_i} \right ) \nabla^2 n_{i1} = 0.
\end{equation}

The dispersion relation for an ion acoustic wave is quite simple, namely
\begin{equation}
    w^2 - k^2 v_s^2 = 0,
\end{equation}
where
\begin{equation}
    v_s = \sqrt{ \frac{Z \gamma_e k_B T_{e0} + \gamma_i k_B T_{i0}}{m_i} }.
\end{equation}
...
\begin{itemize}
    \item The flow is homentropic.
    \item $n_{i0}$, $n_{e0}$, $p_{i0}$, and $p_{e0}$ depend on $\xvec$ only.
    \item $\uvec_{i0}$, $\uvec_{e0}$, $\Evec_0$, and $\Bvec_0$ are zero.
    \item Quasi-neutrality for the base flow, $Zn_{i0} = n_{e0}$. 
    \item The density and pressure are stable, $n_{e1} = n_{i1} = p_{e1} = p_{i1} = 0$.
    \item The ions are stationary, $\uvec_{i1} = 0$.
\end{itemize}
Given the assumptions above, $\rho_e = e(Zn_i - n_e)$ becomes $\rho_e = 0$, whereas $\Jvec = e \left (Z n_i \uvec_i - n_e \uvec_e \right )$ simplifies to $\Jvec = -e n_{e0} \uvec_{e1}$. Combining Maxwell's equations to form wave equations for $\Evec = \Evec(\xvec,t)$ and $\Bvec = \Bvec(\xvec,t)$, as done for \cref{eq:electrodynamics_E_wave_eq,eq:electrodynamics_B_wave_eq} but without assuming $\Jvec = 0$, we get
\begin{equation}
    \label{eq:lpi_E_wave_eq}
    \frac{\partial^2 \Evec}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Evec + \frac{1}{\epsilon_0} \frac{\partial \Jvec}{\partial t} = 0,
\end{equation}
\begin{equation}
    \label{eq:lpi_B_wave_eq}
    \frac{\partial^2 \Bvec}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Bvec - \frac{1}{\epsilon_0} \nabla \times \Jvec = 0.
\end{equation}
We'll now split the vectors into transverse (zero divergence, finite curl) and longitudinal (zero curl, finite divergence) components. We'll label this decomposition as $\Evec = \Evec_t + \Evec_l$, $\Bvec = \Bvec_t + \Bvec_l$, $\Jvec = \Jvec_t + \Jvec_l$, etc., where the $t$ underscript denotes transverse and $l$ denotes longitudinal. Since $\nabla \cdot \Evec = \nabla \cdot \Bvec = 0$, $\Evec_l = \Bvec_l = 0$. Additionally, $\nabla \times \Jvec$ simplifies to $\nabla \times \Jvec_t$. Thus, the wave equations above become
\begin{equation*}
    \frac{\partial^2 \Evec_t}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Evec_t + \frac{1}{\epsilon_0} \frac{\partial \Jvec}{\partial t} = 0,
\end{equation*}
\begin{equation*}
    \frac{\partial^2 \Bvec_t}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Bvec_t - \frac{1}{\epsilon_0} \nabla \times \Jvec_t = 0.
\end{equation*}
If we take the divergence of the electric-field wave equation above, we get $\nabla \cdot \Jvec = 0$, which implies $\Jvec_l = 0$. In a subsequent section, we'll look at a case where $\Jvec_l \ne 0$. For this case, we have to assume that there is a slight imbalance in the charges, so that $\rho_e \ne 0$, and thus through Gauss' law we get $\nabla \cdot \Evec \ne 0$. This last expression then implies $\Evec_l \ne 0$. An additional term containing $\nabla \cdot \Evec_l$ shows up in the electric wave equation, which balances the $\partial \Jvec_l/\partial t$ term. This longitudinal current follows from electron plasma waves. The wave equations are now expressed as
\begin{equation}
...
\begin{equation}
    \frac{\partial^2 \Bvec_t}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Bvec_t - \frac{1}{\epsilon_0} \nabla \times \Jvec_t = 0,
\end{equation}
which for consistency we re-write as 
\begin{equation}
    \frac{\partial^2 \Evec_{1,t}}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Evec_{1,t} + \frac{1}{\epsilon_0} \frac{\partial \Jvec_{1,t}}{\partial t} = 0,
\end{equation}
\begin{equation}
    \frac{\partial^2 \Bvec_{1,t}}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Bvec_{1,t} - \frac{1}{\epsilon_0} \nabla \times \Jvec_{1,t} = 0.
\end{equation}

We now need to find expressions for $\partial \Jvec_{1,t} / \partial t$ and $\nabla \times \Jvec_{1,t}$. Expanding the electron momentum equation, we get
\begin{multline*}
    \frac{\partial}{\partial t} \left [ m_e \left ( n_{e0} + n_{e1} \right ) \left ( \uvec_{e0} + \uvec_{e1} \right ) \right ] + \nabla \cdot \left [ m_e \left ( n_{e0} + n_{e1} \right ) \left ( \uvec_{e0} + \uvec_{e1} \right ) \left ( \uvec_{e0} + \uvec_{e1} \right ) \right ] \\
    + e \left ( n_{e0} + n_{e1} \right ) \left [ \left ( \Evec_0 + \Evec_1 \right ) + \left ( \uvec_{e0} + \uvec_{e1} \right ) \times \left ( \Bvec_0 + \Bvec_1 \right ) \right ] = -\nabla \left ( p_{e0} + p_{e1} \right ),
\end{multline*}
which simplifies to 
\begin{equation}
    \label{eq:em_waves_mom_linearized_pressure}
    \frac{\partial m_e n_{e0} \uvec_{e1}}{\partial t} + e n_{e0} \Evec_1 = - \nabla p_{e0}.
\end{equation}
The transverse component of the above is
\begin{equation}
    \label{eq:em_waves_mom_linearized_pressure_transverse}
    \frac{\partial \uvec_{e1,t}}{\partial t} + \frac{e}{m_e} \Evec_{1,t} = 0.
\end{equation}
As mentioned before, the current is given by $\Jvec = \Jvec_1 = -e n_{e0} \uvec_{e1}$, or, in terms of its transverse component, $\Jvec_{1,t} = -e n_{e0} \uvec_{e1,t}$. Using this and \cref{eq:em_waves_mom_linearized_pressure_transverse} in the wave equations, we get
\begin{equation}
    \frac{\partial^2 \Evec_{1,t}}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Evec_{1,t} + w_{pe}^2 \Evec_{1,t} = 0,
\end{equation}
\begin{equation}
    \frac{\partial^2 \Bvec_{1,t}}{\partial t^2} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \Bvec_{1,t} + \nabla \times \left ( \frac{e n_{e0}}{\epsilon_0} \uvec_{e1,t} \right ) = 0.
\end{equation}

Define the electric-field and magnetic-field waves as 
\begin{equation}
    \label{eq:lpi_E_wave_def}
    \Evec_{1,t} = \bar{\Evec}_{1,t} e^{- iwt},
\end{equation}
\begin{equation}
    \label{eq:lpi_B_wave_def}
    \Bvec_{1,t} = \bar{\Bvec}_{1,t} e^{- iwt},
\end{equation}
where $\bar{\Evec}_{1,t} = \bar{\Evec}_{1,t}(\xvec)$ and $\bar{\Bvec}_{1,t} = \bar{\Bvec}_{1,t}(\xvec)$. Given this wave form, the electric-field wave equation becomes
\begin{equation}
    \label{eq:em_E_wave_expanded}
    -w^2 \bar{\Evec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \bar{\Evec}_{1,t} + w_{pe}^2 \bar{\Evec}_{1,t} = 0.
\end{equation}
If we also introduce $\uvec_{e1,t} = \bar{\uvec}_{e1,t} e^{-iwt}$, then we get for the magnetic-field wave equation
\begin{equation}
    -w^2 \bar{\Bvec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \bar{\Bvec}_{1,t} + \nabla \times \left ( \frac{e n_{e0}}{\epsilon_0} \bar{\uvec}_{e1,t} \right ) = 0.
\end{equation}
\Cref{eq:em_waves_mom_linearized_pressure_transverse} now gives 
\begin{equation}
    \label{eq:em_waves_momentum_wave_form}
    \bar{\uvec}_{e1,t} = -i \frac{e}{m_e w} \bar{\Evec}_{1,t},
\end{equation}
and thus the magnetic-field wave equation becomes
\begin{equation}
    -w^2 \bar{\Bvec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \bar{\Bvec}_{1,t} - i \nabla \times \left ( \frac{w_{pe}^2}{w} \bar{\Evec}_{1,t} \right ) = 0.
\end{equation}
Using the vector identity $\nabla \times (f \Fvec) = f \nabla \times \Fvec + \nabla f \times \Fvec$ we get
\begin{equation}
    -w^2 \bar{\Bvec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \bar{\Bvec}_{1,t} - i \frac{w_{pe}^2}{w} \nabla \times \bar{\Evec}_{1,t} -i \frac{1}{w} \nabla w_{pe}^2 \times \bar{\Evec}_{1,t}  = 0.
\end{equation}
We now use Fraday's law, that is 
\begin{equation}
    \nabla \times \Evec_{1,t} = -\frac{\partial \Bvec_{1,t}}{\partial t},
\end{equation}
Using the pre-determined wave forms, this becomes
\begin{equation}
    \nabla \times \bar{\Evec}_{1,t} = i w \bar{\Bvec}_{1,t}.
\end{equation}
Thus, the magnetic-field wave equation can be written as 
\begin{equation}
    -w^2 \bar{\Bvec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \bar{\Bvec}_{1,t} +  w_{pe}^2 \bar{\Bvec}_{1,t} -i \frac{1}{w} \nabla w_{pe}^2 \times \bar{\Evec}_{1,t}  = 0.
\end{equation}
Similarly, we'll now use Ampere's law with Maxwell's correction, that is
\begin{equation}
    \nabla \times \Bvec_{1,t} = -\mu_0 e n_{e0} \uvec_{e1,t} + \mu_0 \epsilon_0 \frac{\partial \Evec_{1,t}}{\partial t}.
\end{equation}
Using the pre-determined wave forms, this becomes
\begin{equation}
    \nabla \times \bar{\Bvec}_{1,t} = - \mu_0 e n_{e0} \bar{\uvec}_{e1,t} - i w \mu_0 \epsilon_0 \bar{\Evec}_{1,t}.
\end{equation}
Given \cref{eq:em_waves_momentum_wave_form} the above is written as 
\begin{equation}
    \nabla \times \bar{\Bvec}_{1,t} = i \frac{w_{pe}^2}{w} \epsilon_0 \mu_0 \bar{\Evec}_{1,t} - i w \mu_0 \epsilon_0 \bar{\Evec}_{1,t},
\end{equation}
or 
\begin{equation}
    \nabla \times \bar{\Bvec}_{1,t} = i \epsilon_0 \mu_0 \left ( \frac{w_{pe}^2}{w} - w \right ) \bar{\Evec}_{1,t}.
\end{equation} 
Using this in the magnetic-field wave equation we finally get 
\begin{equation}
    \label{eq:em_B_wave_expanded}
    -w^2 \bar{\Bvec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \nabla^2 \bar{\Bvec}_{1,t} +  w_{pe}^2 \bar{\Bvec}_{1,t} - \frac{1}{\epsilon_0 \mu_0} \frac{1}{w_{pe}^2 - w^2} \nabla w_{pe}^2 \times \left ( \nabla \times \bar{\Bvec}_{1,t} \right ) = 0.
\end{equation} 
It is interesting to compare the newly derived electromagnetic wave equations \cref{eq:em_E_wave_expanded,eq:em_B_wave_expanded} against those in free space. The fact that $\uvec_{e1} \ne 0$ leads to a current in the plasma, which in turn accounts for the extra terms with $w_{pe}^2$ in front. The fact that $n_{e0}$ is space dependent accounts for the last term in the magnetic-field wave equation.

If we introduce the dielectric function $\epsilon = 1 - w_{pe}^2 / w^2$, and make use of $c^2 = \epsilon_0 \mu_0$, then the electromagnetic wave equations are written as 
\begin{equation}
    \label{eq:em_E_wave_dielectric}
    \nabla^2 \bar{\Evec}_{1,t} + \frac{w^2}{c^2} \epsilon \bar{\Evec}_{1,t} = 0.
\end{equation}
\begin{equation}
    \label{eq:em_B_wave_dielectric}
    \nabla^2 \bar{\Bvec}_{1,t} + \frac{w^2}{c^2} \epsilon \bar{\Bvec}_{1,t} + \frac{1}{\epsilon} \nabla \epsilon \times \left ( \nabla \times \bar{\Bvec}_{1,t} \right ) = 0.
\end{equation} 
...
\begin{align}
    ...
    \nabla \cdot \Fvec_t &= 0.
\end{align}
Assume vector functions are of the following form
\begin{equation}
...
\end{align}
The first says $\hat{\Fvec}_l$ is parallel to $\kvec$ and the second says $\hat{\Fvec}_t$ is orthogonal to $\kvec$. Thus, $\hat{\Fvec}_l \cdot \hat{\Fvec}_t = 0$. We will often have situations where $\nabla \times \Fvec = \nabla \cdot \Fvec = 0$. That is, $\kvec \times \hat{\Fvec} = \kvec \cdot \hat{\Fvec} =  0$, which means $\hat{\Fvec} = 0$ and thus $\Fvec = 0$. In other words,
\begin{equation}
    \label{eq:em_plasma_null_vector}
    \nabla \times \Fvec = \nabla \cdot \Fvec = 0 \to \Fvec = 0.
\end{equation}

Consider the electric and magnetic fields as well as the scalar and vector potentials, which satisfy
\begin{equation}
    \label{eq:em_plasma_E_potential}
    \Evec = -\nabla \phi - \frac{\partial \Avec}{\partial t},
\end{equation}
\begin{equation}
    \label{eq:em_plasma_B_potential}
    \Bvec = \nabla \times \Avec.
\end{equation}
...
\begin{equation}
    \nabla \cdot \Bvec = \nabla \cdot \Bvec_l + \nabla \cdot \Bvec_t = \nabla \cdot \Bvec_l = 0.
\end{equation}
However, by definition, $\nabla \times \Bvec_l = 0$ as well. Thus, using \cref{eq:em_plasma_null_vector}, we have $\Bvec_l = 0$. The same argument applies to the vector potential, and thus $\Avec_l = 0$. For the electric field, we have
\begin{equation}
    \nabla \cdot \Evec = \nabla \cdot \Evec_l + \nabla \cdot \Evec_t = \nabla \cdot \Evec_l = \nabla \cdot \left ( -\nabla \phi \right ),
\end{equation}
where we used \cref{eq:em_plasma_E_potential} for the last equality. In other words, we have
\begin{equation}
...
\end{equation}
Thus, using \cref{eq:em_plasma_null_vector}, we have $\Evec_l = -\nabla \phi$. A similar argument can be used to show $\Evec_t = -\partial \Avec / \partial t$. Our goal in this section will be to determine equations for $\Evec_l$, $\Evec_t$ and $\Bvec_t$.
...
\end{equation}
Using \cref{eq:em_plasma_null_vector}, we conclude
\begin{equation}
    \label{eq:em_plasma_longitudinal_J}
    \frac{\partial \nabla \phi}{\partial t} = \frac{1}{\epsilon_0} \Jvec_l.
\end{equation}
This gives the equation for $\Evec_l$.

Both $\Evec_t$ and $\Bvec_t$ can be extracted from $\Avec_t$, so now we proceed to find an equation for the transverse vector potential. Ampere's law with Maxwell's correction gives
\begin{equation}
    \nabla \times \left ( \nabla \times \Avec_t \right ) = \mu_0 \Jvec + \mu_0 \epsilon_0 \frac{\partial \Evec}{\partial t}.
\end{equation}
The above is re-written as
\begin{equation}
    \nabla \left ( \nabla \cdot \Avec_t \right ) - \nabla^2 \Avec_t = \mu_0 \Jvec + \mu_0 \epsilon_0 \left ( -\frac{\partial \nabla \phi}{\partial t} - \frac{\partial^2 \Avec_t}{\partial t^2} \right ),
\end{equation}
which gives
\begin{equation}
    \frac{\partial^2 \Avec_t}{\partial t} - \frac{1}{\mu_0 \epsilon_0} \nabla^2 \Avec_t = \frac{1}{\epsilon_0} \Jvec - \frac{\partial \nabla \phi}{\partial t}.
\end{equation}
Expanding the current density as $\Jvec = \Jvec_l + \Jvec_t$, and using \cref{eq:em_plasma_longitudinal_J}, we get
\begin{equation}
    \frac{\partial^2 \Avec_t}{\partial t} - \frac{1}{\mu_0 \epsilon_0} \nabla^2 \Avec_t = \frac{1}{\epsilon_0} \Jvec_t.
\end{equation}
Taking the time derivative of the above gives ... 
Taking the curls of the above gives ...
