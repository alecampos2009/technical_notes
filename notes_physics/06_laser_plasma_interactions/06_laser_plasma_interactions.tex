...
\end{align}
We'll assume that for any vector function $\Gvec = \Gvec(\xvec,t)$, we can express it as
\begin{equation}
    \label{eq:em_general_wave_form}
    \Gvec = \hat{\Gvec} \exp \left [i\left ( \kvec \cdot \xvec - wt \right ) \right ],
\end{equation}
These are the so-called plane waves. In the above, $\hat{\Gvec}$, $\kvec$, and $w$ are all constant. Also note that $\hat{\Gvec}$ is a complex vector, where the real and complex components point in the same direction. In terms of notation, we'll use a different subscript in the wave vector and frequencies of different waves. For example, we'll use $\kvec_e$ for electron-plasma waves, $\kvec_i$ for ion-acoustic waves, $\kvec_L$ for laser waves, and $\kvec_s$ for scattered waves. Similarly for the frequencies $w_e$, $w_i$, $w_L$, $w_s$.

We note that
\begin{equation}
    \nabla \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] = i \kvec \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ].
\end{equation}
Given the identity $\nabla \times (f \Avec) = f (\nabla \times \Avec ) + (\nabla f) \times \Avec$, if a function has no curl we can show that
\begin{align}
    \label{eq:ltw_no_curl}
    0 &= \nabla \times \Gvec \nonumber \\
    &= \nabla \times \left ( \hat{\Gvec} \exp \left [i \left ( \kvec \cdot \xvec - wt \right ) \right ] \right ) \nonumber \\
    &= \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] \nabla \times \hat{\Gvec} + \nabla \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] \times \hat{\Gvec} \nonumber \\
    &= i \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] \kvec \times \hat{\Gvec}.
\end{align}
Given the identity $\nabla \cdot (f\Avec) = f ( \nabla \cdot \Avec) + (\nabla f) \cdot \Avec$, if a function has no divergence we can show that
\begin{align}
    \label{eq:ltw_no_div}
    0 &= \nabla \cdot \Gvec \nonumber \\
    &= \nabla \cdot \left ( \hat{\Gvec} \exp \left [i \left ( \kvec \cdot \xvec - wt \right ) \right ] \right ) \nonumber \\
    &= \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] \nabla \cdot \hat{\Gvec} + \nabla \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] \cdot \hat{\Gvec} \nonumber \\
    &= i \exp \left [ i \left (\kvec \cdot \xvec - wt \right ) \right ] \kvec \cdot \hat{\Gvec}.
\end{align}
Thus, since by definition $\Fvec_l$ has no curl and $\Fvec_t$ has no divergence, \cref{eq:ltw_no_curl,eq:ltw_no_div} give
\begin{align}
    \kvec \times \hat{\Fvec}_l = 0 \nonumber \\
    \kvec \cdot \hat{\Fvec}_t = 0.
\end{align}

The first expression above says $\hat{\Fvec}_l$ is parallel to $\kvec$ and the second says $\hat{\Fvec}_t$ is orthogonal to $\kvec$. Thus, $\hat{\Fvec}_l \cdot \hat{\Fvec}_t = 0$. We will often have situations where $\nabla \times \Fvec = \nabla \cdot \Fvec = 0$, which by its own does not imply $\Fvec = 0$. However, using \cref{eq:ltw_no_curl,eq:ltw_no_div}, this translates to to $\kvec \times \hat{\Fvec} = \kvec \cdot \hat{\Fvec} =  0$. The latter equality states that $\kvec$ and $\hat{\Fvec}$ are orthogonal, that is, the angle between them is $90^\circ$. The former equality leads to $|\hat{\Fvec}| \sin(90^\circ) = 0$, which in turn means $\hat{\Fvec} = 0$. As a result, the entire vector $\Fvec$ is equal to zero. In other words,
\begin{equation}
    \label{eq:ltw_general_null_vector}
    \nabla \times \Fvec = \nabla \cdot \Fvec = 0 \to \Fvec = 0.
\end{equation}

We note that a slightly more general expression than \cref{eq:em_general_wave_form,} can be used for most of the analysis in subsequent sections. Given a wave vector $\kvec = [k_x, k_y, k_z]$ such that $k_x = k_x(x)$, $k_y = k_y(y)$, and $k_z = k_z(z)$, the more general function takes the form
\begin{equation}
...
\end{equation}
For the above, we now have $\hat{\Gvec} = \hat{\Gvec}(\xvec,t)$, that is, the vector coefficient is no longer constant. However, $\hat{\Gvec}$ can't be of any general form. We enforce the constraint that if $\nabla \times \Gvec = 0$, then $\nabla \times \hat{\Gvec} = 0$, and similarly, if $\nabla \cdot \Gvec = 0$, then $\nabla \cdot \hat{\Gvec} = 0$.
We note that
\begin{multline}
...
\end{multline}
Thus, equivalent derivations to those shown in \cref{eq:ltw_no_curl,eq:ltw_no_div} allow us to again obtain $\kvec \times \hat{\Fvec}_l = 0$ and $\kvec \cdot \hat{\Fvec}_t = 0$. Similarly, for a function that has $\nabla \times \Fvec = \nabla \cdot \Fvec = 0$, \cref{eq:ltw_no_curl,eq:ltw_no_div} allows us to express this as $\kvec \times \hat{\Fvec} = \kvec \cdot \hat{\Fvec} = 0$, and thus $\Fvec = 0$. We end with the cautionary note that thus fur we've been working with first gradients of $\Gvec$. Second gradients of $\Gvec$ do not lead to the same results when using \cref{eq:em_general_wave_form} vs \cref{eq:em_more_general_wave_form}.
...
For ion-acoustic waves, we can write \cref{eq:ia_waves_mom_linearized} in spectral form to obtain
\begin{equation}
...
\begin{equation}
    ...
    \frac{\partial^2 \Avec}{\partial t} + w_{pe}^2 \Avec - c_0^2 \nabla^2 \Avec = 0.
\end{equation}
We note that $n_{e0}$ is only time independent, that is, it is still allowed to vary across space. As a result, $w_{pe}^2$ is also allowed to vary across space. Making use of the wave's functional dependence on time, i.e.\@ $\Avec(\xvec,t) = \tilde{\Avec}(\xvec) \exp(-iwt)$, the time derivative in \cref{eq:semp_general_trans_vec_pot_complete} evaluates such that one obtains
\begin{equation*}
...
\end{equation}
Making use of the wave's functional dependence on time, i.e.\@ $\Evec(\xvec,t) = \tilde{\Evec}(\xvec) \exp(-iwt)$, the time derivative in \cref{eq:semp_general_trans_E_complete} evaluates such that one obtains
\begin{equation*}
...
\end{equation}
Making use of the wave's functional dependence on time, i.e.\@ $\Bvec(\xvec,t) = \tilde{\Bvec}(\xvec) \exp(-iwt)$, the time derivative in \cref{eq:semp_general_trans_B_complete} evaluates such that one obtains
\begin{equation*}
...
\end{equation*}
Making use of the wave's functional dependence on time, i.e.\@ $\Avec(\xvec,t) = \tilde{\Avec}(\xvec) \exp(-iwt)$, the expression $\Evec_t = -\partial \Avec / \partial t$ gives
\begin{equation*}
    \Evec_t = iw \Avec.
\end{equation*}
