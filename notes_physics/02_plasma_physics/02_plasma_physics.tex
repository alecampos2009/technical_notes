...
%--------------------------------------------
\subsection{Simplification for isentropic plasmas}
%--------------------------------------------
...
\begin{equation}
    p_{\alpha 1} = \gamma_\alpha n_{\alpha 1} k_B T_\alpha,
\end{equation}
...
\end{equation*}
Since $p_{i0}/n_{i0} = k_B T_i$ and $p_{e0}/n_{e0} = k_B T_e$, we have
\begin{equation}
...
The physical domain will be one dimensional with length $L$, that is, $[-L/2,L/2]$. A voltage $V$ is applied across this domain and thus the boundary condition for the electric potential becomes $\phi(-L/2) - \phi(L/2) = V$. We'll also enforce the condition $\phi(0) = 0$ to fix the arbitrary constant of the potential. 
...
Now let's fill up the physical space with a plasma consisting of electrons and ions. The equations for this case are

\begin{equation}
    \label{eq:debye_def}
    \lambda_{D\alpha} = \left ( \frac{\epsilon_0 k_B T_\alpha}{n_{\alpha0} e^2} \right )^{1/2}
\end{equation}
...
%--------------------------------------------
In \cref{sec:ep_waves} we assumed $w_{pe} < w$ in \cref{eq:ep_waves_pre_dispersion} to obtain a dispersion relation for electron-plasma waves. We now consider the cases $w_{pe} > w$ and $w_{pe} = w$ to better understand the role played by the plasma frequency $w_{pe}$.
...
%--------------------------------------------
\subsection{Momentum exchange collision frequency $\nu_{ei}$}
%--------------------------------------------

%--------------------------------------------
\subsection{Electron-ion thermal coupling $w_{ei}$}
%--------------------------------------------
