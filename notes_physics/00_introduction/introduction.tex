...
\begin{enumerate}
    ...
    \item We can re-write \cref{eq:sf_alternate_p_ki} as $\hat{p}_{k,i} = (\gamma - 1) \rho_k e_{k,i}$ to compute $\hat{p}_{k,i}$.

    \item We can re-write \cref{eq:sf_alternate_p_ke} as $\hat{p}_{k,e} = (\gamma - 1) \rho_k Y_{k,e} e_{k,e}$ to compute $\hat{p}_{k,e}$.
    ...
    \item Using $\hat{p}_{k,i}$ and $\hat{n}_{k,i}$ in \cref{eq:sf_mm_ms_eos_ion_hat} we get $T_{k,i}$.

    \item Using $\hat{p}_{k,e}$ and $\hat{n}_{k,e}$ in \cref{eq:sf_mm_ms_eos_elec_hat} we get $T_{k,e}$.
\end{enumerate}
