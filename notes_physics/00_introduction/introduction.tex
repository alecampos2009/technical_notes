...
%--------------------------------------------
\subsection{Adding the volume fraction}
%--------------------------------------------
In this section we do a switch from variables, which includes going from $p_{k,i}$ and $p_{k,e}$ to $\hat{p}_{k,i}$ and $\hat{p}_{k,e}$. Consider the thermal EOS shown in \cref{eq:sf_mm_ms_eos_ion}, which we repeat below for convenience
\begin{equation}
...
\end{equation*}
The above represent the thermal EOS's where the reference volume is $V_k$ rather than $V$, which is the standard for EOS's in a multi-material problem. Similarly, we can re-write \cref{eq:sf_alternate_p_ki} as
\begin{equation*}
    \hat{p}_{k,i} = (\gamma - 1) \rho_k e_{k,i},
\end{equation*}
and \cref{eq:sf_alternate_p_ke} as 
\begin{equation*}
    \hat{p}_{k,e} = (\gamma - 1) \rho_k Y_{k,e} e_{k,e}.
\end{equation*}
...
\end{equation}
\begin{equation}
    \label{eq:sf_mm_ms_therm_eos_ion_hat}
    \hat{p}_{k,i} = \hat{n}_{k,i} k_B T_{k,i},
\end{equation}
\begin{equation}
    \label{eq:sf_mm_ms_therm_eos_elec_hat}
    \hat{p}_{k,e} = \hat{n}_{k,e} k_B T_{k,e},
\end{equation}
\begin{equation}
    \label{eq:sf_mm_ms_calo_eos_ion_hat}
    \hat{p}_{k,i} = (\gamma - 1) \rho_k e_{k,i},
\end{equation}
\begin{equation}
    \label{eq:sf_mm_ms_calo_eos_elec_hat}
    \hat{p}_{k,e} = (\gamma - 1) \rho_k Y_{k,e} e_{k,e}.
\end{equation}

Some extra notes on this model:
\begin{enumerate}
    \item As before the mass transport equation gives $\rho Y_k Y_{k,i,s}$. Summing over all $s$ gives $\rho Y_k$. Dividing $\rho Y_k Y_{k,i,s}$ by $\rho Y_k$ gives $Y_{k,i,s}$. Summing $\rho Y_k$ over all $k$ gives $\rho$. Dividing $\rho Y_k$ by $\rho$ gives $Y_k$.
    ...
    \item Using $\rho_k$ and $e_{k,i}$ in \cref{eq:sf_mm_ms_calo_eos_ion_hat} we get $\hat{p}_{k,i}$.

    \item Using $\rho_k$ and $Y_{k,e} e_{k,e}$ in \cref{eq:sf_mm_ms_calo_eos_elec_hat} we get $\hat{p}_{k,e}$.
    ...
    \item We can compute $h_{k,e}$ using the first equality in \cref{eq:sf_alternate_hke_rho}, which we repeat below
    \begin{equation*}
        Y_{k,e} h_{k,e} = Y_{k,e} e_{k,e} + \frac{\hat{p}_{k,e}}{\rho_k}.
    \end{equation*}

    \item We can compute $\hat{n}_{k,i,s}$ using \cref{eq:mm_mass_number_densities_i_hat}, which we repeat below
    \begin{equation*}
        m_{k,i,s} \hat{n}_{k,i,s} = \rho_k Y_{k,i,s}.
    \end{equation*}

    \item We can compute $\hat{n}_{k,i}$ using \cref{eq:sf_vars_hatn_ki}, that is, $\hat{n}_{k,i,s} = \sum_s^{N_k} \hat{n}_{k,i,s}$.
    
    \item The quasi-neutrality assumption in \cref{eq:sf_quasi_neutrality} is repeated below
    \begin{equation}
        \sum_s^{N_k} Z_{k,i,s} n_{k,i,s} = n_{k,e}.
    \end{equation}
    Using \cref{eq:sf_vars_def_hatn_kis} and \cref{eq:sf_vars_def_hatn_ke} this is re-written as
    \begin{equation}
        \sum_s^{N_k} Z_{k,i,s} \hat{n}_{k,i,s} = \hat{n}_{k,e},
    \end{equation}
    and can be used to compute $\hat{n}_{ke}$.

    \item Using $\hat{p}_{k,i}$ and $\hat{n}_{k,i}$ in \cref{eq:sf_mm_ms_therm_eos_ion_hat} we get $T_{k,i}$.

    \item Using $\hat{p}_{k,e}$ and $\hat{n}_{k,e}$ in \cref{eq:sf_mm_ms_therm_eos_elec_hat} we get $T_{k,e}$.
\end{enumerate}
