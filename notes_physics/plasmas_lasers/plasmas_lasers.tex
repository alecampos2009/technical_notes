...
We consider $N$ ion species plus one electron species. A particular ion species is denoted using the $s$ subscript, and the electron species with the $e$ subscript. Additionally, we assume 
\begin{enumerate}
    \item No sources for ions ($\hat{S}_s = \hat{\Mvec}_s = \hat{Q}_s = 0$ for all $s$) and electrons ($\hat{S}_e = \hat{\Mvec}_e = \hat{Q}_e = 0$).
\end{enumerate}
Thus, the governing equations are
\begin{equation}
    \label{eq:mf_np1fluid_ni}
    \frac{\partial n_s}{\partial t} + \nabla \cdot \left ( n_s \uvec_s \right ) = 0,
\end{equation}
\begin{equation}
...
\begin{equation}
    ...
    \Jvec = e \left(\sum_s^N Z_s n_s \uvec_s - n_e \uvec_e \right),
\end{equation}
\begin{equation}
    \label{eq:mf_np1fluid_mass_density}
    \rho_q = e \left( \sum_s^N Z_s n_s - n_e \right),
\end{equation}
\begin{equation}
...
%--------------------------------------------
 The volume occupied by all ion species at a single point is denoted as $V$, and the mass occupied by all ion species at a single point as $m$. Similarly $V_s$ and $m_s$ are the volume and mass, respectively, occupied by the $s$ ion species at a single point. Finally, $V_e$ and $m_e$ are the volume and mass, respectively, occupied by the electrons at a single point.

For the single-fluid formulation, we'll use the following assumptions
\begin{enumerate}
    \item $m_e \approx 0$ since $m_e \ll m_s$. \label{eq:sf_no_e_mass} 
    \item Quasi-neutrality, i.e.\@ $\sum_s^N Z_s n_s = n_e$. \label{eq:sf_quasi_neutrality}
\end{enumerate}
...
\subsection{Mass}
%--------------------------------------------
Multiplying \cref{eq:mf_np1fluid_ni} by $M_s$ gives
\begin{equation*}
    \frac{\partial \rho Y_s}{\partial t} + \nabla \cdot \left( \rho Y_s \uvec_s \right) = 0.
\end{equation*}
...
\begin{equation}
    \frac{\partial \rho Y_s}{\partial t} + \nabla \cdot \left( \rho Y_s \uvec \right) = -\nabla \cdot \jvec_s.
\end{equation}

Multiplying \cref{eq:mf_np1fluid_ni} by $Z_s$ and summing over all $s$ gives
\begin{equation*}
    \frac{\partial}{\partial t} \sum_s^N Z_s n_s + \nabla \cdot \left( \sum_s^N Z_s n_s \uvec_s \right) = 0.
\end{equation*}
...
\begin{equation*}
    \nabla \cdot \left( \sum_s^N Z_s n_s \uvec_s - n_e \uvec_e \right) = 0,
\end{equation*}
...
\begin{multline*}
    \frac{\partial \rho Y_s \uvec_s}{\partial t} + \nabla \cdot \left( \rho Y_s \uvec_s \uvec_s \right) -e \left( Z_s n_s - n_e \right) \Evec - e \left( Z_s n_s \uvec_s - n_e \uvec_e \right) \times \Bvec = \\ \nabla \cdot \left( \sigmavec_s + \sigmavec_e \right) + \Rvec_s + \Rvec_e.
\end{multline*}
Summing over all $s$ gives
\begin{multline*}
    \frac{\partial}{\partial t} \sum_s^N \rho Y_s \uvec_s + \nabla \cdot \sum_s^N \rho Y_s \uvec_s \uvec_s -e \left( \sum_s^N Z_s n_s - n_e \right) \Evec - e \left( \sum_s^N Z_s n_s \uvec_s - n_e \uvec_e \right) \times \Bvec = \\ \nabla \cdot \left( \sum_s^N \sigmavec_s + \sigmavec_e \right) + \sum_s^N \Rvec_s + \Rvec_e.
\end{multline*}
...
\begin{equation*}
    e n_e ( \Evec + \uvec_e \times \Bvec ) = e n_e \Evec + e n_e \uvec_e \times \Bvec = e n_e \Evec + \left( e \sum_s^N Z_s n_s \uvec_s - \Jvec \right) \times \Bvec,
\end{equation*}
and thus the electron momentum equation becomes
\begin{equation*}
    e n_e \Evec + e \sum_s^N Z_s n_s \uvec_s \times \Bvec = \Jvec \times \Bvec + \nabla \cdot \sigmavec_e + \Rvec_e,
\end{equation*}
Using \cref{eq:sf_mass_number_densities} and the definition of $\jvec_s$, the left-hand side can be written as
\begin{align*}
    e n_e \Evec + e \sum_s^N Z_s n_s \uvec_s \times \Bvec &= e n_e \Evec + e \sum_s^N \left[ Z_s n_s \uvec + Z_s n_s \left( \uvec_s - \uvec \right) \right] \times \Bvec \\
    &= e n_e \Evec + e \sum_s^N \left( Z_s n_s \uvec + \frac{Z_s \jvec_s}{M_s} \right) \times \Bvec \\
    &= e n_e \Evec + e \sum_s^N Z_s n_s \uvec \times \Bvec + e \sum_s^N \frac{Z_s \jvec_s}{M_s} \times \Bvec \\
    &= e n_e \left( \Evec + \uvec \times \Bvec \right) + e \sum_s^N \frac{Z_s \jvec_s}{M_s} \times \Bvec.
\end{align*}
Thus, the electron momentum equation finally becomes
\begin{equation*}
    e n_e \left( \Evec + \uvec \times \Bvec \right) = \Jvec \times \Bvec + \nabla \cdot \sigmavec_e + \Rvec_e - e \sum_s^N \frac{Z_s \jvec_s}{M_s} \times \Bvec,
\end{equation*}
or
\begin{equation}
    \Evec + \uvec \times \Bvec = \frac{1}{e n_e} \left( \Jvec \times \Bvec + \nabla \cdot \sigmavec_e + \Rvec_e \right) - \sum_s^N \frac{Z_s \jvec_s}{n_e M_s} \times \Bvec.
\end{equation}
...
\begin{equation*}
    \frac{\partial}{\partial t} \left( \frac{3}{2} p_s \right) + \nabla \cdot \left( \frac{3}{2} p_s \uvec \right) = \sigmavec_s : \nabla \uvec - \nabla \cdot \qvec_s + Q_s - \nabla \cdot \left( \frac{3}{2} \frac{p_s}{\rho Y_s} \jvec_s \right) + \sigmavec_s : \nabla \left( \frac{1}{\rho Y_s} \jvec_s \right).
\end{equation*}
We re-write the above as
\begin{equation*}
    \frac{\partial}{\partial t} \left( \frac{3}{2} p_s \right) + \nabla \cdot \left( \frac{3}{2} p_s \uvec \right) = \sigmavec_s : \nabla \uvec - \nabla \cdot \qvec_s + Q_s - \nabla \cdot \left( \frac{3}{2} \frac{p_s}{\rho Y_s} \jvec_s \right) + \nabla \left( \frac{\sigmavec_s \cdot \jvec_s}{\rho Y_s} \right) - \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s.
\end{equation*}
or
\begin{equation}
    \frac{\partial}{\partial t} \left( \frac{3}{2} p_s \right) + \nabla \cdot \left( \frac{3}{2} p_s \uvec \right) = \sigmavec_s : \nabla \uvec - \nabla \cdot \qvec_s + Q_s - \nabla \cdot \left[ \left( \frac{3}{2} p_s \Ivec - \sigmavec_s \right) \cdot \frac{\jvec_s}{\rho Y_s}  \right] - \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s.
\end{equation}
...
Thus far we've been working with plasmas that behave as ideal gasses with three degrees of freedom (the three translational degrees of freedom). For ideal gasses we also have $p_s = (\gamma - 1) \rho Y_s e_s$ for all ions and $p_e = (\gamma = 1) \rho Y_e e_e$ for electrons, where $e_s$ and $e_e$ are the internal energies of ions and electrons, respectively. Thus, given that $\gamma = 5/3$, the internal energy equations become
\begin{equation*}
    \frac{\partial \rho Y_s e_s}{\partial t} + \nabla \cdot \left( \rho Y_s e_s \uvec \right) = \sigmavec_s : \nabla \uvec - \nabla \cdot \qvec_s + Q_s - \nabla \cdot \left[ \left( \rho Y_s e_s \Ivec - \sigmavec_s \right) \cdot \frac{\jvec_s}{\rho Y_s}  \right] - \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s.
\end{equation*}
\begin{equation*}
...
We can also use the expressions for the stress tensors, that is $\sigmavec_s = -p_s \Ivec + \tvec_s$ and $\sigmavec_e = -p_e \Ivec + \tvec_e$, to obtain
\begin{equation*}
    \frac{\partial \rho Y_s e_s}{\partial t} + \nabla \cdot \left( \rho Y_s e_s \uvec \right) = \sigmavec_s : \nabla \uvec - \nabla \cdot \qvec_s + Q_s -\nabla \cdot \left[ \left( e_s + \frac{p_s}{\rho Y_s} \right) \jvec_s \right] + \nabla \cdot \left( \tvec_s \cdot \frac{\jvec_s}{\rho Y_s} \right) - \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s.
\end{equation*}
\begin{equation*}
...
Since
\begin{equation*}
    \frac{p_s}{\rho Y_s} = \frac{\eta_s \hat{p}_s}{\rho Y_s} = \frac{\hat{p}}{\rho_s},
\end{equation*}
and $h_s = e_s + \hat{p}_s / \rho_s$ we have for the ions
\begin{equation*}
    \frac{\partial \rho Y_s e_s}{\partial t} + \nabla \cdot \left( \rho Y_s e_s \uvec \right) = \sigmavec_s : \nabla \uvec - \nabla \cdot \qvec_s + Q_s -\nabla \cdot \left( h_s \jvec_s \right) + \nabla \cdot \left( \tvec_s \cdot \frac{\jvec_s}{\rho Y_s} \right) - \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s.
\end{equation*}
...
\begin{equation}
    e = \sum_s^N Y_s e_s + Y_e e_e,
\end{equation}
so that adding up the internal energy equations gives
\begin{multline*}
    \frac{\partial \rho e}{\partial t} + \nabla \cdot \left( \rho e \uvec \right) = \left( \sum_s^N \sigmavec_s + \sigmavec_e \right): \nabla \uvec - \nabla \cdot \left( \sum_s^N \qvec_s + \qvec_e \right) + \sum_s^N Q_s + Q_e \\
    - \nabla \cdot \left( \sum_s^N h_s \jvec_s + h_e \jvec_e \right) + \nabla \cdot \left( \sum_s^N \tvec_s \cdot \frac{\jvec_s}{\rho Y_s} + \tvec_e \cdot \frac{\jvec_e}{\rho Y_e} \right) - \left( \sum_s^N \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s + \frac{\jvec_e}{\rho Y_e} \cdot \nabla \sigmavec_e \right).
\end{multline*}
Using \cref{eq:sf_sum_energy_collisions} we have
\begin{equation}
    \sum_s^N Q_s + Q_e = - \left( \sum_s^N \uvec_s \cdot \Rvec_s + \uvec_e \cdot \Rvec_e \right).
\end{equation}
Thus, the total internal energy equation becomes
\begin{multline*}
    \frac{\partial \rho e}{\partial t} + \nabla \cdot \left( \rho e \uvec \right) = \left( \sum_s^N \sigmavec_s + \sigmavec_e \right): \nabla \uvec - \nabla \cdot \left( \sum_s^N \qvec_s + \qvec_e \right) - \left( \sum_s^N \uvec_s \cdot \Rvec_s + \uvec_e \cdot \Rvec_e \right) \\
    - \nabla \cdot \left( \sum_s^N h_s \jvec_s + h_e \jvec_e \right) + \nabla \cdot \left( \sum_s^N \tvec_s \cdot \frac{\jvec_s}{\rho Y_s} + \tvec_e \cdot \frac{\jvec_e}{\rho Y_e} \right) - \left( \sum_s^N \frac{\jvec_s}{\rho Y_s} \cdot \nabla \sigmavec_s + \frac{\jvec_e}{\rho Y_e} \cdot \nabla \sigmavec_e \right).
\end{multline*}
