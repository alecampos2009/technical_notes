...
%------------------------------------------------------------------------
The starting point are the $N+1$ fluid equations of \cref{sec:mf_np1fluid_equations}. For the single-fluid formulation, we'll use the following assumptions
\begin{enumerate}
...
We define the density of an ion species $\rho_\alpha$ as
\begin{equation}
    \rho_\alpha = m_\alpha n_\alpha.
\end{equation}
and the total density $\rho$ as
\begin{equation}
    \rho = \sum_{\alpha}^N \rho_\alpha.
\end{equation}
The total velocity $\uvec$ is given by
\begin{equation}
    \uvec = \frac{\sum_{\alpha}^N \rho_\alpha \uvec_\alpha}{\sum_{\alpha}^N \rho_\alpha}.
\end{equation}

\subsubsection{Mass conservation}
Multiplying \cref{eq:mf_np1fluid_ni} by $m_\alpha$ gives
\begin{equation*}
    \frac{\partial \rho_\alpha}{\partial t} + \nabla \cdot \left( \rho_\alpha \uvec_\alpha \right) = 0.
\end{equation*}
We introduce the ion species diffusion vector $\jvec_\alpha = \rho_\alpha \left( \uvec_\alpha - \uvec \right)$ and thus write the above as
\begin{equation}
    \frac{\partial \rho_\alpha}{\partial t} + \nabla \cdot \left( \rho_\alpha \uvec \right) = -\nabla \cdot \jvec_\alpha.
\end{equation}
...
\end{equation*}
Subtracting \cref{eq:mf_np1fluid_ne} from the above gives
\begin{equation*}
...
\begin{multline*}
    \frac{\partial \rho_\alpha \uvec_\alpha}{\partial t} + \nabla \cdot \left( \rho_\alpha \uvec_\alpha \uvec_\alpha \right) -e \left( Z_\alpha n_\alpha - n_e \right) \Evec - e \left( Z_\alpha n_\alpha \uvec_\alpha - n_e \uvec_e \right) \times \Bvec = \\ \nabla \cdot \left( \sigmavec_\alpha + \sigmavec_e \right) + \Rvec_\alpha + \Rvec_e.
\end{multline*}
Summing over all $\alpha$ gives
\begin{multline*}
    \frac{\partial}{\partial t} \sum_\alpha^N \rho_\alpha \uvec_\alpha + \nabla \cdot \sum_\alpha^N \rho_\alpha \uvec_\alpha \uvec_\alpha -e \left( \sum_\alpha^N Z_\alpha n_\alpha - n_e \right) \Evec - e \left( \sum_\alpha^N Z_\alpha n_\alpha \uvec_\alpha - n_e \uvec_e \right) \times \Bvec = \\ \nabla \cdot \left( \sum_\alpha^N \sigmavec_\alpha + \sigmavec_e \right) + \sum_\alpha^N \Rvec_\alpha + \Rvec_e.
\end{multline*}
...
\begin{equation*}
    \frac{\partial}{\partial t} \sum_\alpha^N \rho_\alpha \uvec_\alpha + \nabla \cdot \sum_\alpha^N \rho_\alpha \uvec_\alpha \uvec_\alpha - \Jvec \times \Bvec = \nabla \cdot \left( \sum_\alpha^N \sigmavec_\alpha + \sigmavec_{\alpha_e} \right) + \sum_\alpha^N \Rvec_\alpha + \Rvec_e.
\end{equation*}
Due to \cref{eq:coll_cons_particles,eq:coll_cons_momentum} we have
\begin{equation}
    \sum_\alpha^N \Rvec_\alpha + \Rvec_e = 0.
\end{equation}
We now write the momentum equation as
\begin{equation*}
    \frac{\partial}{\partial t} \sum_\alpha^N \rho_\alpha \uvec_\alpha + \nabla \cdot \sum_\alpha^N \left[ \rho_\alpha \uvec_\alpha \uvec + \rho_\alpha \uvec_\alpha \left( \uvec_\alpha - \uvec \right) \right] - \Jvec \times \Bvec = \nabla \cdot \left( \sum_\alpha^N \sigmavec_\alpha + \sigmavec_{\alpha_e} \right).
\end{equation*}
Using the definitions of $\rho$, $\uvec$, and $\jvec_\alpha$ the above becomes
\begin{equation}
...
\end{equation*}
Using the definition of $\jvec_\alpha$, the left-hand side can be written as
\begin{align*}
...
Due to the first assumption, the Maxwell equations \cref{eq:mf_2fluid_maxwell_1,eq:mf_2fluid_maxwell_2,eq:mf_2fluid_maxwell_3,eq:mf_2fluid_maxwell_4,eq:mf_2fluid_curr_density,eq:mf_2fluid_mass_density} are now written as
\begin{equation}
...
The two conservation of mass equations \cref{eq:mf_2fluid_ni,eq:mf_2fluid_ne} will lead to two single-fluid equations. The first is obtained by multiplying \cref{eq:mf_2fluid_ni} by $m_i$, and the second is obtained by multiplying the ion and electron mass equations by $e$ and then subtracting. The results are
\begin{equation}
...
\end{equation}
Assuming quasi-neutrality, the definition of the current given in \cref{eq:mf_2fluid_curr_density} is now
\begin{equation}
...
The two conservation of energy equations will also lead to two single-fluid equations. Each is evaluated using the single-fluid variables. As part of this derivation, we first rewrite the ion and electron internal energy \cref{eq:mf_2fluid_iei,eq:mf_2fluid_iee} as
\begin{equation}
\label{eq:twof_ie}
