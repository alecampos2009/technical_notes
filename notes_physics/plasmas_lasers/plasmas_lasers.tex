...
\end{equation}
\begin{equation}
\rho_q = \sum_\alpha Z_\alpha e \int f_\alpha \, d\vvec.
\end{equation}
In the above, 
\begin{itemize}
...
%--------------------------------------------
\subsection{Summary of fluid equations}
%--------------------------------------------
...
%--------------------------------------------
\begin{enumerate}
    \item Species $\alpha$ are separated into two categories: ions ($\alpha_i$) and electrons ($\alpha_e$).
    \item No sources ($\hat{S}_\alpha$, $\hat{\Mvec}_\alpha$, $\hat{Q}_\alpha$).
\end{enumerate}
Thus, the governing equations are
\begin{equation}
    \label{eq:mf_ms_ni}
    \frac{\partial n_{\alpha_i}}{\partial t} + \nabla \cdot \left ( n_{\alpha_i} \uvec_{\alpha_i} \right ) = 0,
\end{equation}
\begin{equation}
    \label{eq:mf_ms_ne}
    \frac{\partial n_{\alpha_e}}{\partial t} + \nabla \cdot \left ( n_{\alpha_e} \uvec_{\alpha_e} \right ) = 0,
\end{equation}
\begin{equation}
    \label{eq:mf_ms_ui}
    \frac{\partial m_{\alpha_i} n_{\alpha_i} \uvec_{\alpha_i}}{\partial t} + \nabla \cdot \left( m_{\alpha_i} n_{\alpha_i} \uvec_{\alpha_i} \uvec_{\alpha_i} \right) - Z_{\alpha_i} e n_{\alpha_i} ( \Evec + \uvec_{\alpha_i} \times \Bvec ) = \nabla \cdot \sigmavec_{\alpha_i} + \Rvec_{\alpha_i},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_ue}
    \frac{\partial m_e n_{\alpha_e} \uvec_{\alpha_e}}{\partial t} + \nabla \cdot \left ( m_e n_{\alpha_e} \uvec_{\alpha_e} \uvec_{\alpha_e} \right) + e n_{\alpha_e} ( \Evec + \uvec_{\alpha_e} \times \Bvec ) = \nabla \cdot \sigmavec_{\alpha_e} + \Rvec_{\alpha_e},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_iei}
    \frac{\partial}{\partial t} \left ( \frac{3}{2} p_{\alpha_i} \right ) + \nabla \cdot \left ( \frac{3}{2} p_{\alpha_i} \uvec_{\alpha_i} \right ) = \sigmavec_{\alpha_i} : \nabla \uvec_{\alpha_i} - \nabla \cdot \qvec_{\alpha_i} + Q_{\alpha_i},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_iee}
    \frac{\partial}{\partial t} \left ( \frac{3}{2} p_{\alpha_e} \right ) + \nabla \cdot \left ( \frac{3}{2} p_{\alpha_e} \uvec_{\alpha_e} \right ) = \sigmavec_{\alpha_e} : \nabla \uvec_{\alpha_e} - \nabla \cdot \qvec_{\alpha_e} + Q_{\alpha_e},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_maxwell_3}
    \nabla \cdot \Evec = \frac{\rho_q}{\epsilon_0},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_maxwell_4}
    \nabla \cdot \Bvec = 0,
\end{equation}
\begin{equation}
    \label{eq:mf_ms_maxwell_1}
    \nabla \times \Evec = -\frac{ \partial \Bvec}{\partial t},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_maxwell_2}
    \nabla \times \Bvec = \mu_0 \Jvec + \mu_0 \epsilon_0 \frac{\partial \Evec}{\partial t},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_curr_density}
    \Jvec_\alpha = \sum_\alpha e (Z_{\alpha_i} n_{\alpha_i} \uvec_{\alpha_i} - n_{\alpha_e} \uvec_{\alpha_e}),
\end{equation}
\begin{equation}
    \label{eq:mf_ms_mass_density}
    \rho_{q,\alpha} = \sum_\alpha e (Z_{\alpha_i} n_{\alpha_i} - n_{\alpha_e}),
\end{equation}
\begin{equation}
    \label{eq:mf_ms_curr_density_total}
    \Jvec = \sum_\alpha \Jvec_\alpha,
\end{equation}
\begin{equation}
    \label{eq:mf_ms_mass_density_total}
    \rho_q = \sum_\alpha \rho_{q,\alpha}.
\end{equation}
We'll also introduce the following simple equations of state
\begin{equation}
    \label{eq:mf_ms_eos_ion}
    p_{\alpha_i} = n_{\alpha_i} k_B T_{\alpha_i},
\end{equation}
\begin{equation}
    \label{eq:mf_ms_eos_elec}
    p_{\alpha_e} = n_{\alpha_e} k_B T_{\alpha_e}.
\end{equation}
...
\label{sec:mf_ss_equations}
In this section we simplify the approach by considering a single ion species and a single electron species. Thus, the governing equations become
\begin{equation}
    \label{eq:mf_ss_ni}
...
\end{equation}
\begin{equation}
    \label{eq:mf_ss_maxwell_3}
    \nabla \cdot \Evec = \frac{\rho_q}{\epsilon_0},
\end{equation}
\begin{equation}
    \label{eq:mf_ss_maxwell_4}
    \nabla \cdot \Bvec = 0,
\end{equation}
\begin{equation}
    \label{eq:mf_ss_maxwell_1}
    \nabla \times \Evec = -\frac{ \partial \Bvec}{\partial t},
\end{equation}
\begin{equation}
    \label{eq:mf_ss_maxwell_2}
    \nabla \times \Bvec = \mu_0 \Jvec + \mu_0 \epsilon_0 \frac{\partial \Evec}{\partial t},
\end{equation}
\begin{equation}
    \label{eq:mf_ss_curr_density}
    \Jvec = e (Z n_i \uvec_i - n_e \uvec_e),
\end{equation}
\begin{equation}
    \label{eq:mf_ss_mass_density}
    \rho_q = e (Z n_i - n_e).
\end{equation}
We'll also introduce the following simple equations of state
\begin{equation}
    \label{eq:mf_ss_eos_ion}
    p_i = n_i k_B T_i,
\end{equation}
\begin{equation}
    \label{eq:mf_ss_eos_elec}
    p_e = n_e k_B T_e.
\end{equation}
...
%------------------------------------------------------------------------
Define the density of an ion species $\rho_{\alpha_i}$ as
\begin{equation}
    \rho_{\alpha_i} = m_{\alpha_i} n_{\alpha_i}.
\end{equation}
Additionally, define the total velocity $\uvec$ as
\begin{equation}
    \uvec = \sum_\alpha \frac{m_{\alpha_i} n_{\alpha_i} u_{\alpha_i} + m_{\alpha_e} n_{\alpha_e} u_{\alpha_e}}{m_{\alpha_e} n_{\alpha_e} + m_{\alpha_e} n_{\alpha_e}} = \sum_{\alpha_i} \frac{m_{\alpha_i} n_{\alpha_i} u_{\alpha_i}}{m_{\alpha_i} n_{\alpha_i}}.
\end{equation}
In the above, we assumed $m_{\alpha_e} \ll m_{\alpha_i}$ and thus simplify the sum to only those species that are ions.

Multiplying \cref{eq:mf_ms_ni} by $m_{\alpha_i}$ gives
\begin{equation*}
    \frac{\partial \rho_{\alpha_i}}{\partial t} + \nabla \cdot \left( \rho_{\alpha_i} \uvec_{\alpha_i} \right) = 0.
\end{equation*}
We introduce the ion species diffusion vector $\jvec_{\alpha_i} = \rho_{\alpha_i} \left( \uvec_{\alpha_i} - \uvec \right)$ and thus write the above as
\begin{equation}
    \frac{\partial \rho_{\alpha_i}}{\partial t} + \nabla \cdot \left( \rho_{\alpha_i} \uvec \right) = -\nabla \cdot \jvec_{\alpha_i}.
\end{equation}
We'll also assume quasi-neutrality for each ion-electron species pair, that is, $\rho_{q,\alpha} = 0$, which leads to
\begin{equation}
    Z_{\alpha_i} n_{\alpha_i} = n_{\alpha,e}.
\end{equation}
Multiplying \cref{eq:mf_ms_ni} by $Z_{\alpha_i}$ and then subtracting from it \cref{eq:mf_ms_ne} gives
\begin{equation*}
    \nabla \cdot \left( Z_{\alpha_i} n_{\alpha_i} \uvec_{\alpha_i} - n_{\alpha_e} \uvec_{\alpha_e} \right) = 0.
\end{equation*}
Multiplying by $e$ gives
\begin{equation}
    \nabla \cdot \Jvec_\alpha = 0.
\end{equation}
...
%--------------------------------------------
To describe the Debye length we'll use the governing equations from \cref{sec:mf_ss_equations} as well as the following assumptions
\begin{enumerate}
...
Due to the first assumption, the Maxwell equations \cref{eq:mf_ss_maxwell_1,eq:mf_ss_maxwell_2,eq:mf_ss_maxwell_3,eq:mf_ss_maxwell_4,eq:mf_ss_curr_density,eq:mf_ss_mass_density} are now written as
\begin{equation}
...
The two conservation of mass equations \cref{eq:mf_ss_ni,eq:mf_ss_ne} will lead to two single-fluid equations. The first is obtained by multiplying \cref{eq:mf_ss_ni} by $m_i$, and the second is obtained by multiplying the ion and electron mass equations by $e$ and then subtracting. The results are
\begin{equation}
...
\end{equation}
Assuming quasi-neutrality, the definition of the current given in \cref{eq:mf_ss_curr_density} is now
\begin{equation}
...
The two conservation of energy equations will also lead to two single-fluid equations. Each is evaluated using the single-fluid variables. As part of this derivation, we first rewrite the ion and electron internal energy \cref{eq:mf_ss_iei,eq:mf_ss_iee} as
\begin{equation}
\label{eq:twof_ie}
