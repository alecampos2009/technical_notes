...
%------------------------------------------------------------------------
The starting point are the $N+1$ fluid equations of \cref{sec:mf_np1fluid_equations}. Before that, however, we introduce the following variables. $V$ is the volume occupied by all ion species at a single point, and $m$ is the mass occupied by all ion species at a single point. Similarly $V_\alpha$ and $m_\alpha$ are the volume and mass, respectively, occupied by the $\alpha$ ion species at a single point. Finally, $V_e$ and $m_e$ are the volume and mass, respectively, occupied by the electrons at a single point.
...
\begin{enumerate}
    ...
    \item $V_e \approx 0$ since $V_e \ll V_\alpha$. \label{eq:sf_no_e_vol}
...
\end{equation*}
We introduce the ion species diffusion vector $\jvec_\alpha = \rho Y_\alpha \left( \uvec_\alpha - \uvec \right)$ and thus write the above as
\begin{equation}
