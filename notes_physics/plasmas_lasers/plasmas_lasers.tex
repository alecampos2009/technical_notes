...
\begin{equation}
    \rho = m/V.
\end{equation}
The volume fractions $\eta_{i,s}$ and $\eta_e$ are 
\begin{equation}
    \eta_{i,s} = \frac{V_{i,s}}{V} \qquad \eta_e = \frac{V_e}{V}.
\end{equation}
The mass fractions $Y_{i,s}$ and $Y_e$ are
\begin{equation}
...
New densities are also introduced, namely $\rho_{i,s}$ and $\rho_e$, which are defined as
\begin{equation}
...
In terms of the number densities $n_{i,s}$ and $n_e$, we have
\begin{equation}
    \label{eq:sf_mass_number_densities}
    \rho Y_{i,s} = M_{i,s} n_{i,s} \qquad \rho Y_e = M_e n_e.
\end{equation}
where $M_{i,s}$ is the atomic weight of ion species $s$ and $M_e$ the weight of an electron. The total velocity $\uvec$ is given by
\begin{equation}
